...
def test_cut1_routes_are_exact_stage8_and_not_preflight_owned(monkeypatch: Any, tmp_path: Path) -> None:
    for branch, s in SCOPES.items():
        m=monkeypatch;ss=sorted(s);m.setattr(stage8,"cut1_transition_charges",lambda:(0,{}))
        m.setattr(stage8,"citation_parity_charge",lambda:1200);assert route(m,branch,ss)==[]
        assert route(m,branch,[e:="forbidden/outside.txt"])==[f"Stage 8 changed file outside the allowlist: {e}"]
        if branch==CP:m.setattr(stage8,"citation_parity_charge",lambda:1201);assert len(route(m,branch,ss[1:]))==2
...
def test_issue366_contract_rejects_partial_scope_and_content_mutations(monkeypatch: Any) -> None:
    ...
    b=sorted(F);p=s.R434;v={**s.LIMITS434,p[11]:100,p[12]:100};cases:Any=((5601,{}),(851,{b[0]:851}),(1201,{p[9]:1201}),
        (0,v),(201,{p[11]:101,p[12]:100}));z(s,"issue434_charges",lambda:(0,{}));P=sp.run;assert route(m,B,b[1:])
    for n,x in cases:z(s,"issue434_charges",lambda n=n,x=x:(n,x));A=s.A434;R=REPO;u=p[11];assert route(m,B,b)
    a={x:(REPO/x).read_bytes()for x in A};f=s.issue434_artifact_findings;assert not f(a)and f({**a,A[0]:b"x"})and f({})
    q=s.check_issue434_verifier;z(s,"run",pytest.fail);z(s,"issue434_artifact_findings",lambda _:["x"]);q([])
    C=q.__code__;assert "R434" not in C.co_names and p[9] in C.co_consts and not P(["python3","-S",u],cwd=R).returncode
