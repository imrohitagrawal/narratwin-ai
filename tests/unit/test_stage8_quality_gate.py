...
def test_cut1_routes_are_exact_stage8_and_not_preflight_owned(monkeypatch: Any, tmp_path: Path) -> None:
    for branch, s in SCOPES.items():
        (m:=monkeypatch).setattr(stage8,"citation_parity_charge",lambda:1200)
        m.setattr(stage8,"cut1_transition_charges",lambda:(0,{})); assert route(m,branch,sorted(s))==[]
...
def test_issue383_and_issue384_routes_are_exact_and_complete(monkeypatch: Any) -> None:
    expected_383 = {
    ...
    assert stage8.ISSUE383_ASSET_BRANCH == "stage8-383-presenter-assets"
    assert stage8.ISSUE383_ASSET_FILES == expected_383
    assert stage8.ISSUE384_ASSET_ROUTE_BRANCH == "stage8-384-presenter-asset-route"
    assert stage8.ISSUE384_ASSET_ROUTE_FILES == expected_384
    assert stage8.ISSUE384_BASE == "48fc32a2689c9bbc03742d774f3eadb8a500dafc"
    monkeypatch.setattr(stage8, "issue383_route_base", lambda: "base")
    monkeypatch.setattr(stage8, "route_text_charges", lambda *_: (0, {}))
    monkeypatch.setattr(stage8, "route_binary_sizes", lambda *_: {path: 1 for path in stage8.ISSUE383_BINARY_FILES})
    for branch, paths in (
        (stage8.ISSUE383_ASSET_BRANCH, expected_383),
        (stage8.ISSUE384_ASSET_ROUTE_BRANCH, expected_384),
    ):
        assert route(monkeypatch, branch, sorted(paths)) == []
        missing = sorted(paths)[0]
        assert route(monkeypatch, branch, sorted(paths - {missing})) == [
            f"Issue #{383 if branch == stage8.ISSUE383_ASSET_BRANCH else 384} route is missing required path: {missing}"
        ]
        assert route(monkeypatch, branch + "-retry", sorted(paths))
        assert route(monkeypatch, branch.upper(), sorted(paths))


def test_issue383_route_rejects_text_and_binary_budget_overages(monkeypatch: Any) -> None:
    monkeypatch.setattr(stage8, "issue383_route_base", lambda: "base")
    monkeypatch.setattr(stage8, "route_binary_sizes", lambda *_: {path: 1 for path in stage8.ISSUE383_BINARY_FILES})
    full = sorted(stage8.ISSUE383_ASSET_FILES)
    monkeypatch.setattr(stage8, "route_text_charges", lambda *_: (701, {}))
    assert route(monkeypatch, stage8.ISSUE383_ASSET_BRANCH, full) == [
        "Issue #383 charge 701 exceeds 700."
    ]
    monkeypatch.setattr(stage8, "route_text_charges", lambda *_: (1, {"tests/unit/test_cut1_presenter_assets.py": 261}))
    assert route(monkeypatch, stage8.ISSUE383_ASSET_BRANCH, full) == [
        "Issue #383 charge for tests/unit/test_cut1_presenter_assets.py exceeds 260."
    ]
    monkeypatch.setattr(stage8, "route_text_charges", lambda *_: (1, {"docs/STATUS.md": 161}))
    assert route(monkeypatch, stage8.ISSUE383_ASSET_BRANCH, full) == [
        "Issue #383 charge for docs/STATUS.md exceeds 160."
    ]
    monkeypatch.setattr(stage8, "route_text_charges", lambda *_: (0, {}))
    monkeypatch.setattr(
        stage8,
        "route_binary_sizes",
        lambda *_: {path: 500001 if "myra" in path else 1 for path in stage8.ISSUE383_BINARY_FILES},
    )
    assert route(monkeypatch, stage8.ISSUE383_ASSET_BRANCH, full) == [
...
def test_issue384_route_rejects_its_fixed_base_budgets(monkeypatch: Any) -> None:
    full = sorted(stage8.ISSUE384_ASSET_ROUTE_FILES)
    monkeypatch.setattr(stage8, "route_text_charges", lambda *_: (901, {}))
    assert route(monkeypatch, stage8.ISSUE384_ASSET_ROUTE_BRANCH, full) == [
        "Issue #384 charge 901 exceeds 900."
    ]
    monkeypatch.setattr(stage8, "route_text_charges", lambda *_: (1, {"scripts/quality/check_stage8_docs.py": 401}))
    assert route(monkeypatch, stage8.ISSUE384_ASSET_ROUTE_BRANCH, full) == [
        "Issue #384 charge for scripts/quality/check_stage8_docs.py exceeds 400."
    ]
    monkeypatch.setattr(stage8, "route_text_charges", lambda *_: (1, {"tests/unit/test_stage8_quality_gate.py": 351}))
    assert route(monkeypatch, stage8.ISSUE384_ASSET_ROUTE_BRANCH, full) == [
        "Issue #384 charge for tests/unit/test_stage8_quality_gate.py exceeds 350."
    ]


def test_route_text_charges_uses_additions_deletions_and_larger_complete_snapshot(monkeypatch: Any) -> None:
    calls: list[list[str]] = []
    ...
    def fake(args: list[str]) -> sp.CompletedProcess[str]:
        ...
        output = "4\t3\tdocs/file.md\n" if "--cached" in args else "5\t4\tdocs/file.md\n"
        return sp.CompletedProcess(args, 0, output, "")

    monkeypatch.setattr(stage8, "run", fake)
    assert stage8.route_text_charges("base", {"docs/file.md"}) == (9, {"docs/file.md": 9})
    ...
    assert ["git", "ls-files", "-z", "--others", "--exclude-standard", "--", "docs/file.md"] in calls


def test_route_text_charges_rejects_untracked_failed_malformed_binary_and_wrong_path(monkeypatch: Any) -> None:
    completed = sp.CompletedProcess
    cases = (
        (completed([], 0, "docs/file.md\0", ""), completed([], 0, "", ""), "untracked"),
        (completed([], 1, "", "failed"), completed([], 0, "", ""), "failed"),
        (completed([], 0, "", ""), completed([], 0, "-\t1\tdocs/file.md\n", ""), "malformed or binary"),
    ...
    for untracked, diff, message in cases:
        ...
        monkeypatch.setattr(stage8, "run", fake)
        assert message in str(pytest.raises(RuntimeError, stage8.route_text_charges, "base", {"docs/file.md"}).value)


def test_issue383_route_base_uses_current_main_and_fails_closed(monkeypatch: Any) -> None:
    completed = sp.CompletedProcess
    calls: list[list[str]] = []

    def good(args: list[str]) -> sp.CompletedProcess[str]:
        calls.append(args)
        return completed(args, 0, "base\n", "")

    monkeypatch.setattr(stage8, "run", good)
    assert stage8.issue383_route_base() == "base"
    assert calls == [["git", "merge-base", "origin/main", "HEAD"]]
    monkeypatch.setattr(stage8, "run", lambda args: completed(args, 1, "", "failed"))
    assert "merge-base" in str(pytest.raises(RuntimeError, stage8.issue383_route_base).value)


def test_route_binary_sizes_rejects_missing_symlink_directory_empty_and_oversized(
    ...
    assert "missing" in str(pytest.raises(RuntimeError, stage8.route_binary_sizes, {path}).value)
    target.mkdir()
    assert "regular" in str(pytest.raises(RuntimeError, stage8.route_binary_sizes, {path}).value)
    target.rmdir(); target.write_bytes(b"")
    assert "empty" in str(pytest.raises(RuntimeError, stage8.route_binary_sizes, {path}).value)
    target.write_bytes(b"x" * 500001)
    assert stage8.route_binary_sizes({path}) == {path: 500001}
    target.unlink(); source = tmp_path / "source.webp"; source.write_bytes(b"x"); target.symlink_to(source)
    assert "regular" in str(pytest.raises(RuntimeError, stage8.route_binary_sizes, {path}).value)
