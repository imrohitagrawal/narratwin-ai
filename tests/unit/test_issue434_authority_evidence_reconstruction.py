...
def test_all_ordered_pin_references_use_the_pin_hash_domain() -> None:
    manifest, blobs = reconstruction_bundle(b"fixture-only payload"); document = json.loads(manifest); acceptance = json.loads(blobs[document["acceptanceRootPinSetHash"]]); acceptance["rootContentHashes"] = sorted([*acceptance["rootContentHashes"], "f" * 64]); acceptance_bytes = trust.canonical_bytes(acceptance); acceptance_hash = pin_hash(acceptance); current = json.loads(blobs[document["currentRootPinSetHash"]]); current["rootContentHashes"] = sorted([*current["rootContentHashes"], "e" * 64]); current_bytes = trust.canonical_bytes(current); current_hash = pin_hash(current); blobs[acceptance_hash] = acceptance_bytes; blobs[current_hash] = current_bytes; document["acceptanceRootPinReferences"].append(dict(document["acceptanceRootPinReferences"][0], contentHash=acceptance_hash, byteLength=len(acceptance_bytes))); document["currentRootPinReferences"].append(dict(document["currentRootPinReferences"][0], contentHash=current_hash, byteLength=len(current_bytes))); references = [item for name in ("acceptanceRootPinReferences", "currentRootPinReferences", "rootReferences", "keyReferences") for item in document[name]] + [document["envelopeReference"], document["payloadReference"]]; [item.update(ordinal=index) for index, item in enumerate(references)]; document["retainedBlobCount"] = len(references); document["aggregateRetainedByteLength"] = sum(len(blob) for blob in blobs.values()); document["contentHash"] = trust.content_hash(trust.ContentKind.RECONSTRUCTION, "AuthorityEvidenceReconstructionV1", document); result = future("reconstruct_retained_evidence", manifest_bytes=trust.canonical_bytes(document), retained_blobs=blobs, evaluation_time=T10); assert "RETAINED_BLOB_HASH_MISMATCH" not in codes(result)  # noqa: B007, E702
def test_current_pin_failure_does_not_contaminate_history(monkeypatch: pytest.MonkeyPatch) -> None:
    arguments, _ = complete_arguments(); arguments["current_expected_pin_hash"] = "f" * 64; module = importlib.import_module("scripts.quality.issue434_authority_evidence_reconstruction"); root_calls: list[str] = []; original_root = module.validate_closed_root; monkeypatch.setattr(module, "validate_closed_root", lambda **kw: (root_calls.__iadd__([cast(str, kw["expected_phase"])]), original_root(**kw))[1]); result = trusted_result(monkeypatch, arguments); invalid_hash_calls = tuple(root_calls); absent_hash, _ = complete_arguments(); absent_hash["acceptance_expected_pin_hash"] = None; absent_hash_result = trusted_result(monkeypatch, absent_hash); malformed_hash, _ = complete_arguments(); malformed_hash["acceptance_expected_pin_hash"] = {}; malformed_hash_result = trusted_result(monkeypatch, malformed_hash)  # noqa: E702
...
def test_artifact_aliases_and_root_history_topology_fail_closed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert all(cast(Any, result).valid is False for result in artifact_results) and topology.current_verdict is trust.Verdict.CONFLICTING and genesis_result.current_verdict is trust.Verdict.CONFLICTING and malformed_result.current_verdict is trust.Verdict.INVALID
