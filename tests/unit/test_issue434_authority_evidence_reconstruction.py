...
def test_replay_requires_exact_contiguous_predecessor(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    results = [cast(Any, future("validate_evidence_replay_set", envelope_documents=documents)).valid for documents in ((raw,), (first, raw))]; high = trusted_result(monkeypatch, dict(arguments, envelope_bytes=raw))  # noqa: E702
    assert results == [False, False] and high.trusted is False and "EVIDENCE_PREDECESSOR_MISMATCH" in codes(high)
