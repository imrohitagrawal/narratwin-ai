...
def test_replay_requires_exact_contiguous_predecessor(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    results = [cast(Any, future("validate_evidence_replay_set", envelope_documents=documents)).valid for documents in ((raw,), (first, raw))]; calls: list[int] = []; module = importlib.import_module("scripts.quality.issue434_authority_evidence_reconstruction"); original = module._pin_codes; pin_calls: list[int] = []; monkeypatch.setattr(module, "_pin_codes", lambda *a, **kw: (pin_calls.__iadd__([1]), original(*a, **kw))[1]); monkeypatch.setattr(trust, "verify_ed25519_signature", lambda **_: (calls.__iadd__([1]), trust.SignatureResult(True, ()))[1]); high = future("resolve_complete_evidence", **dict(arguments, envelope_bytes=raw))  # noqa: E702
    assert results == [False, False] and cast(Any, high).trusted is False and "EVIDENCE_PREDECESSOR_MISMATCH" in codes(high) and not calls and not pin_calls
