...
`Exact changes` must begin with `#### Reviewer impact summary` and exactly seven
distinct Markdown bullets before `#### Technical change list`. In order, their
exact bold labels are `**Purpose:**`, `**Behavior before/after:**`, `**Who and
what is affected:**`, `**Artifacts/capabilities:**`, `**Operational impact:**`,
`**Scope boundaries:**`, and `**End-to-end impact:**`. Together they state why
the change matters; observable before/after behavior; affected users, systems,
workflows, interfaces, and data; added, changed, removed, or explicitly absent
artifacts and capabilities; runtime/external, persistence, migration,
compatibility, failure, rollback, and rollout effects; explicit exclusions; and
the blocker removed, capability unlocked, and remaining gap. A category with no
effect uses `None` only with a PR-specific reason. Missing, reordered,
...
misleading, copied, incomplete, or keyword-stuffed prose and still owns truth
and clarity.
