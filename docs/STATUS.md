...
| `#280` | Semantically under correction by `#298` | Checkpoint 3A C3A-R3 arbitrary local/demo multilingual closure | PR A-D are merged through PRs `#281`, `#282`, `#283`, and `#284`; prior PR E evidence was later found insufficient because real browser output accepted metadata-only/mock target transcript text. Issue `#298` now carries the corrective semantic closure work: deterministic 25-language local/demo conversion for bounded public-safe synthetic source clauses, hard rejection of metadata-only target text and English fallback, all-25 real browser verification, artifact/report parity, and the memory artifact `docs/reviews/ISSUE_280_SEMANTIC_GAP_MEMORY_2026-07-25.md`. The latest corrective evidence gives Spanish and Hindi browser-visible depth semantics a scoped `SEMANTIC_PASS`, but keeps the broad `R280-AUDIENCE-DEPTH-001` row, depth semantics outside those languages, the full audience-by-depth cross-language matrix, arbitrary runtime project knowledge, and Issue `#280` satisfaction `NOT_PROVEN`. The requirement matrix therefore keeps `issue280SatisfiedByPrE=false` and records only bounded-clause demo implementation. The `#280` outcome must not be treated as semantically satisfied until the remaining requirement is explicitly proven or reviewed/re-scoped. It authorizes no hosted/public demo, provider setup, paid spend, real provider calls, cloned identity runtime, real media, public distribution, arbitrary human-grade or real-world translation quality, provider quality, or production-readiness claim. |
| `#298` | Open | Correct issue `#280` semantic multilingual conversion gap | Branch `phase-1-closure-298-issue280-semantic-multilingual-correction` is limited to repairing the semantic gap found by real UI testing: preserve the session memory file, distinguish semantic executable pass from structural pass in the matrix/gates, record ADR `0040`, reject metadata-only/mock conversion evidence, and prove visible 25-language deterministic local/demo conversion of bounded public-safe synthetic project clauses through backend/API contracts and Playwright UI evidence. It authorizes no provider setup, paid spend, hosted/public demo, real provider calls, cloned identity runtime, real media, public distribution, arbitrary human-grade translation quality, provider quality, or production-readiness claim. |
...
- Issue `#289` tracks the combined security unblock for the PostCSS advisory
  ...
  settled before PR E, and issues `#249` and `#280` remain open.
...
3. Complete issue `#298` as the corrective semantic closure slice for issue
   `#280`. The current load-bearing evidence is the strengthened
   `make issue280-output-correctness` gate plus the memory artifact
   `docs/reviews/ISSUE_280_SEMANTIC_GAP_MEMORY_2026-07-25.md`; it must prove
   browser-visible semantic target transcripts across the 25 supported
   local-demo languages and fail metadata-only/mock/English-fallback success.
   Issue `#249` remains open and C3B remains blocked until issue `#280` is
   semantically satisfied by reviewed/merged evidence or explicitly re-scoped.
4. Keep production,
...
| Date | Change |
|---|---|
| 2026-07-25 | Issue `#298` started on branch `phase-1-closure-298-issue280-semantic-multilingual-correction` after real-browser all-language UI testing found that issue `#280` PR E closure evidence accepted metadata-only/mock target transcripts as multilingual output correctness. Scope is limited to preserving the semantic-gap memory artifact, strengthening the issue `#280` output-correctness gate, rejecting metadata-only/mock/English-fallback evidence, correcting deterministic local/demo semantic conversion for bounded public-safe synthetic project clauses, recording ADR `0040`, installing a repo-level post-implementation execution-verifier guardrail for future implementation PRs, and updating matrix/status/traceability governance. It authorizes no provider setup, paid spend, hosted/public demo, real provider call, cloned identity runtime, real media, public distribution, arbitrary human-grade translation quality, provider quality, production-readiness claim, or Checkpoint 3B implementation. |
