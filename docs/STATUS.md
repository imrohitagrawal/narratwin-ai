...
## Current comparison preparation — Issue 519 / draft PR546

[Issue #519 / PR #546](https://github.com/imrohitagrawal/narratwin-ai/pull/546) prepares the [experiment-first execution package](work/demo-comparison/EXECUTION_PLAN.md), with [decisions](work/demo-comparison/DECISIONS.md), [review/failure evidence](work/demo-comparison/evidence/REVIEWS.md) and [handoff](work/demo-comparison/HANDOFF.md). It preserves the archived proposal and exact canonical inputs; no exploratory A1–A6 amendment, provider qualification or new operation is accepted by this package. PR545 is retired unmerged with its failed attempt preserved. Public research and eligible UI planning are independent of blanket G1/G2/G3 ordering. [Issue #532's owner decision](https://github.com/imrohitagrawal/narratwin-ai/issues/532#issuecomment-5721036635) lifts `INC-G3-URL-001` only for separately reviewed public-research/P1 preparation; account/credential, provider-operation, rights/privacy/lifecycle, spend and broader G3 gates remain separate. #531 accepted-current implementation dependencies are unchanged.

T05 audio acceptance remains complete; T06 has zero accepted real presenter cells and T07/T08 remain incomplete. HeyGen v8/v9 and Pruna P1/P1B are rejected evidence: do not repeat Avatar IV/static-office composition or a third P-Video-Avatar prompt run. The smallest continuous hypothesis remains one newly recorded, consented real-office driver for Pruna `p-video-replace`; Runway Act-Two is conditional only after an explicit PVR unavailable/rejected disposition, and Aleph is only a separately declared scene-edit constituent. The identified VOSU/Instagram reference demonstrates a distinct multi-shot editorial hypothesis rather than a better background prompt, so zero-exposure HFE-1 shot/input planning may proceed in parallel: approximately 55–60% visible presenter and 40–45% genuine product/environmental cutaways under continuous complete narration. Higgsfield Cinema Studio 4.0/Seedance 2.5 and VOSU-hosted Seedance are separate product surfaces; exact graph, canonical-WAV use, rights, reproducibility, web/API parity and an approved Meera reference pack remain unproved. Any later editorial generation starts with one finite P1D diagnostic before complete proof. P-Video-Replace accepts a source MP4 and has no separate WAV input: the exact 27.216125-second PCM16 WAV remains authoritative, while any embedded/restored audio is a separately reviewed and authorized derived representation. No conversion/remux is authorized. Stock Envato/iStock is reference/fallback only, not admitted. The office still remains a style/layout/lighting reference, not the final background. This package grants zero new provider exposure and adds no user-facing capability. Representative full continuous landscape/portrait quality and one complete editorial walkthrough must precede workflow selection; adapter/browser media, captions, playback/export and formal T06–T08 follow only after selection. [#543 / draft PR544](https://github.com/imrohitagrawal/narratwin-ai/pull/544) owns further source corrections and its failed verification evidence; this package neither changes those bytes nor completes G1 certification/adoption. Required hosted checks, independent exact-head review and enforced approval remain separate merge gates. Routine tracker/check/cleanup observations stay in issue/PR comments.
...
- Issue `#434` is the current OWNER-approved, nonactivating Child B route under
  ...
  `87b8504ca8d5e094394343aeaa4ef5bad46133d5` and is normally synchronized,
  without rewrite, with accepted Stage 8 security-prerequisite merge
  `370c52f0a1b618758e2a9a2c28188e75384a5922`. Child A completed with 18 paths
...
- Implementation permission is otherwise closed. The exact amended 16-path Issue `#358`
  ...
  harness and API E2E foundation only. Issue `#257` is closed after PR `#258`
  merged the second Checkpoint 3A child implementation checkpoint for
  executable output-correctness only. Issue `#259` is closed after PR `#260`
  merged the third Checkpoint 3A child implementation checkpoint for executable
  language-quality only. Issue `#261` is closed after PR `#262` merged the
  fourth Checkpoint 3A child implementation checkpoint for executable
  media-artifacts only at `58e6ac473bb2cbcd5e99a64007a1cc862117217c`.
  Issue `#263` is closed after PR `#264` merged the fifth Checkpoint 3A child
  implementation checkpoint for executable access/quota/retention only at
  `de0cdb0c5337a980e478cb3e6b42d2b031909f31`. Issue `#265` is closed after
  PR `#266` merged the sixth Checkpoint 3A child implementation checkpoint for
  executable security/observability only at
  `f79debb641e7198e2d1d41e210ddd537037c7699`. Issue `#267` is closed after
  PR `#268` merged the seventh Checkpoint 3A child implementation checkpoint
  for executable performance only at
  ...
  workflow run `30079561208` passing. Issue `#280` is closed in GitHub but not
  fixed. PR A-D remain historical slices through merged PRs `#281`-`#284`.
  Two canonical payload runs at PR E evidence head
  `f93653e8a11e697c88766b207fb01c18662339d6` completed all 525 combinations
  without translation refusal, retained seven accepted scripts, and emitted one
  visible target body across audiences in all 75 groups. The old 217/308
  aggregate was not reproduced. PR #301 is limited to negative forensic
  containment; it supplies no product repair,
  execution runner, semantic oracle, or closure authority. Issue `#274` is satisfied by its prior
  reviewed PR as the
  ...
  after this child. C3B remains blocked pending later reviewed product work.
  This state
...
| `#249` | Open | Checkpoint 3 tracker after C3-PR1 planning/guardrails | C3-PR1 planning and guardrails completed through merged PR `#250` at `41b262fa2431f55cd1c813eab4071968c1c96ba0`. Issue `#249` remains open after CP1-CP8 and the falsified Issue #280 attempt. PR #301 is negative forensic containment only; later product work requires separate authorization. |
...
| `#259` | Closed | Checkpoint 3A child CP3 language-quality probe | Closed after PR `#260` merged at `16d8cfea1fd14050833a16cc4da827be16289911`. The C3A-CP3 branch added the third executable `make checkpoint3-acceptance` probe for runtime language-quality: approved synthetic knowledge, local API generation, idempotent API replay, coherent walkthrough structure, audience-appropriate English Stage 4 tone, citation readability, `contextRefs`, `claimSupports`, evidence snapshots, placeholder rejection, trivially short output rejection, debug/internal leakage rejection, cross-project language rejection, style-text-without-runtime-evidence rejection, docs/prose/static/canned success rejection, bounded subprocess timeout, and redacted failure summaries. It kept issue `#249` open and left media artifacts, access/quota/retention, security/observability, performance, and real-browser E2E probes planned/non-passing. It authorized no hosted deployment, public URL, provider setup, real provider call, paid spend, cloned identity, public distribution, or production-readiness claim. |
| `#261` | Closed | Checkpoint 3A child CP4 media-artifacts probe | Closed after PR `#262` merged at `58e6ac473bb2cbcd5e99a64007a1cc862117217c`. The C3A-CP4 branch added the fourth executable `make checkpoint3-acceptance` probe for runtime media-artifacts: approved synthetic knowledge, local API generation, Stage 6 translated-script/subtitles/voice-manifest artifacts, Stage 7 synthetic-avatar consent plus demo HTML/render-manifest/video-export-placeholder artifacts, idempotent API replay, artifact MIME/safe-filename/base64/checksum validation, source-run/evaluation/citation/context/claim-support binding, local/mock provider posture, real-media overclaim rejection, cloned-identity overclaim rejection, docs/prose/static/canned success rejection, bounded subprocess timeout, and redacted failure summaries. It kept issue `#249` open and left access/quota/retention, security/observability, performance, and real-browser E2E probes planned/non-passing. It authorized no hosted deployment, public URL, provider setup, real provider call, paid spend, cloned identity, real media binaries, public distribution, or production-readiness claim. |
| `#263` | Closed | Checkpoint 3A child CP5 access/quota/retention probe | Closed after PR `#264` merged at `de0cdb0c5337a980e478cb3e6b42d2b031909f31`. The C3A-CP5 branch added the fifth executable `make checkpoint3-acceptance` probe for runtime access/quota/retention: at least two approved synthetic local projects, knowledge upload/approval/ingestion, grounded walkthrough generation, cross-actor and cross-project access boundaries, mismatched source-run replay rejection, scoped idempotency replay, deterministic upload/prompt/document/local hosted-demo quota limits, terminal-retention replay denial, tombstone evidence, bounded ops/status evidence, docs/prose/static/canned success rejection, bounded subprocess timeout, and redacted failure summaries. It kept issue `#249` open and left security/observability, performance, and real-browser E2E probes planned/non-passing. It authorized no hosted deployment, public URL, provider setup, real provider call, paid spend, cloned identity, real media binaries, public distribution, or production-readiness claim. |
| `#265` | Closed | Checkpoint 3A child CP6 security/observability probe | Closed after PR `#266` merged at `f79debb641e7198e2d1d41e210ddd537037c7699`. The C3A-CP6 branch added the sixth executable `make checkpoint3-acceptance` probe for runtime security/observability: approved synthetic local projects, knowledge upload/approval/ingestion, grounded walkthrough generation, runtime API-visible security controls, missing approval failure, unsafe upload failure, prompt-injection refusal, unsupported claim rejection, cross-project replay rejection, same-payload cross-actor access denial with a reused idempotency key, same-actor idempotency conflict behavior, trace/run/evaluation observability metadata, source/run binding failure, local/mock provider posture, bounded ops/status evidence, docs/prose/static/canned success rejection, bounded subprocess timeout, and redacted failure summaries. It kept issue `#249` open and left performance and real-browser E2E probes planned/non-passing. It authorized no hosted deployment, public URL, provider setup, real provider call, paid spend, cloned identity, real media binaries, public distribution, or production-readiness claim. |
...
| `#258` | Merged | 2026-07-22 | C3A-CP2 executable output-correctness probe for issue `#257`; merged at `a93948434dbc50a62a7b0b86eeef07f4d9f97cd6`. It preserved the CP1 API E2E probe, added local/mock runtime output-correctness evidence and negative false-pass coverage, kept issue `#249` open, and made no provider setup, real provider call, paid spend, hosted deployment, public URL, cloned identity, public distribution, or production-readiness claim. |
| `#260` | Merged | 2026-07-22 | C3A-CP3 executable language-quality probe for issue `#259`; merged at `16d8cfea1fd14050833a16cc4da827be16289911` after latest-head human approval on head `232a8dabf5d957430c3ae334fb6cdc2301b94e4b`, with post-merge main quality workflow run `29937721472` passing. It preserved the CP1 API E2E and CP2 output-correctness probes, added local/mock runtime language-quality evidence and negative false-pass coverage, kept issue `#249` open, and made no provider setup, real provider call, paid spend, hosted deployment, public URL, cloned identity, public distribution, or production-readiness claim. |
| `#262` | Merged | 2026-07-22 | C3A-CP4 executable media-artifacts probe for issue `#261`; merged at `58e6ac473bb2cbcd5e99a64007a1cc862117217c`. It preserved the CP1 API E2E, CP2 output-correctness, and CP3 language-quality probes, added local/mock runtime media-artifact evidence and negative false-pass coverage, kept issue `#249` open, and made no provider setup, real provider call, paid spend, hosted deployment, public URL, cloned identity, real media binary, public distribution, or production-readiness claim. |
| `#264` | Merged | 2026-07-23 | C3A-CP5 executable access/quota/retention probe for issue `#263`; merged at `de0cdb0c5337a980e478cb3e6b42d2b031909f31`. It preserved the CP1 API E2E, CP2 output-correctness, CP3 language-quality, and CP4 media-artifacts probes, added local/mock runtime access/quota/retention evidence and negative false-pass coverage, kept issue `#249` open, and made no provider setup, real provider call, paid spend, hosted deployment, public URL, cloned identity, real media binary, public distribution, or production-readiness claim. |
| `#266` | Merged | 2026-07-23 | C3A-CP6 executable security/observability probe for issue `#265`; merged at `f79debb641e7198e2d1d41e210ddd537037c7699`. It preserved the CP1 API E2E, CP2 output-correctness, CP3 language-quality, CP4 media-artifacts, and CP5 access/quota/retention probes, added local/mock runtime security/observability evidence and negative false-pass coverage, kept issue `#249` open, and made no provider setup, real provider call, paid spend, hosted deployment, public URL, cloned identity, real media binary, public distribution, or production-readiness claim. |
| `#268` | Merged | 2026-07-23 | C3A-CP7 executable performance probe for issue `#267`; merged at `caa8183be7ebf3fa5a3cf34d653727cc5522bf7f`. It preserved the CP1 API E2E, CP2 output-correctness, CP3 language-quality, CP4 media-artifacts, CP5 access/quota/retention, and CP6 security/observability probes, added local/mock runtime performance evidence and negative false-pass coverage, kept issue `#249` open, and made no provider setup, real provider call, paid spend, hosted deployment, public URL, cloned identity, real media binary, public distribution, or production-readiness claim. |
| `#273` | Merged | 2026-07-23 | C3A-CP8 executable real-browser E2E probe for issue `#269`; merged at `0f737c564f9245b66640988573ac04f4432e06d5` with post-merge main quality workflow run `29994103118` passing. It preserved CP1 through CP7, added local/mock real-browser E2E evidence with no success-path interception, kept issue `#249` open, closed issue `#269`, and made no provider setup, real provider call, paid spend, hosted deployment, public URL, cloned identity, real media binary, public distribution, Checkpoint 3B, Checkpoint 3C, or production-readiness claim. |
...
- Issue `#289` tracks the combined security unblock for the PostCSS advisory
  and inherited Stage 8 quality-marker drift because split draft PRs `#286`
  and `#288` are circularly blocked by required CI. Scope is dependency
  metadata plus checker/docs/tests/status only. PR `#284` is now merged and
  settled before the historical PR E attempt; current posture is recorded above.
...
| 2026-07-25 | Issue `#289` started on branch `phase-1-closure-process-289-security-postcss-stage8-gate-unblock` to resolve the circular CI blocker between dependency-security PR `#286` and governance PR `#288`. Scope is limited to PostCSS dependency metadata plus checker/docs/tests/status alignment: `postcss` resolves to a patched version without a Next downgrade, mandatory container scanning remains bound to Trivy, Grype, or the pinned Dockerized scanner path while Docker Scout remains optional, and the Stage 6 voice-provider JSON-manifest marker is documented. Issue `#289` authorizes no product/runtime code, provider setup, paid spend, hosted/public demo, public distribution, production-readiness claim, or issue-closing wording for `#249` or `#280`. |
| 2026-07-24 | Issue `#280` continued as C3A-R3 PR D on branch `phase-1-closure-280-c3a-r3-pr-d-ui-browser-demo-slice` after PR `#283` merged PR C at `09584b264c0f30da3eecd6693829e5bcb071e568` with post-merge main quality workflow run `30095714825` passing. Scope is exact local UI/browser evidence for `/api/v1/checkpoint3/issue280/local-e2e-demo`: bounded synthetic markdown submission, visible stored grounded result, transcript preview/expansion, citations, context refs, claim supports, evaluation metadata, retry/refusal states, info tooltips, keyboard/mobile behavior, and provider-disabled local/mock posture. Issues `#249` and `#280` remain open after PR D unless all remaining R280 rows are later satisfied or reviewed/re-scoped, C3B remains blocked, and PR D authorizes no final issue-closure gate, hosted/public demo, provider setup, paid spend, real provider calls, cloned identity runtime, real media, public distribution, arbitrary real-world translation quality, provider quality, or production-readiness claim. |
...
| 2026-07-23 | Issue `#267` started as C3A-CP7 performance on branch `phase-1-closure-267-c3a-cp7-performance-probe`; the first branch commit contains only `docs/governance/preflights/issue-267.json`. Scope is executable local/mock performance evidence through the API path only, with API E2E, output-correctness, language-quality, media-artifacts, access/quota/retention, and security/observability preserved, real-browser E2E still planned/non-passing, issue `#249` remaining open, and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, real media binaries, public distribution, or production-readiness claim. |
| 2026-07-23 | PR `#264` merged issue `#263` C3A-CP5 access/quota/retention at `de0cdb0c5337a980e478cb3e6b42d2b031909f31`; issue `#263` is closed, issue `#249` remains open, and the next approved action is issue `#265` C3A-CP6 executable security/observability on branch `phase-1-closure-265-c3a-cp6-security-observability`. Scope remains local/mock API evidence only, with performance and real-browser E2E still planned/non-passing and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, real media binaries, public distribution, or production-readiness claim. |
| 2026-07-23 | Issue `#265` started as C3A-CP6 security/observability on branch `phase-1-closure-265-c3a-cp6-security-observability`; the first branch commit contains only `docs/governance/preflights/issue-265.json`. Scope is executable local/mock security/observability evidence through the API path only, with API E2E, output-correctness, language-quality, media-artifacts, and access/quota/retention preserved, later Checkpoint 3A probes still planned/non-passing, issue `#249` remaining open, and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, real media binaries, public distribution, or production-readiness claim. |
| 2026-07-22 | Issue `#263` started as C3A-CP5 access/quota/retention on branch `phase-1-closure-263-c3a-cp5-access-quota-retention`; the first branch commit contains only `docs/governance/preflights/issue-263.json`. Scope is executable local/mock access/quota/retention evidence through the API path only, with API E2E, output-correctness, language-quality, and media-artifacts preserved, later Checkpoint 3A probes still planned/non-passing, issue `#249` remaining open, and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, real media binaries, public distribution, or production-readiness claim. |
| 2026-07-22 | PR `#262` merged issue `#261` C3A-CP4 media-artifacts at `58e6ac473bb2cbcd5e99a64007a1cc862117217c`; issue `#261` is closed, issue `#249` remains open, and the next approved action is issue `#263` C3A-CP5 executable access/quota/retention on branch `phase-1-closure-263-c3a-cp5-access-quota-retention`. Scope remains local/mock API evidence only, with security/observability, performance, and real-browser E2E still planned/non-passing and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, real media binaries, public distribution, or production-readiness claim. |
...
| 2026-07-22 | PR `#258` merged issue `#257` C3A-CP2 output-correctness at `a93948434dbc50a62a7b0b86eeef07f4d9f97cd6`; issue `#257` is closed, issue `#249` remains open, and the next approved action is issue `#259` C3A-CP3 executable language-quality on branch `phase-1-closure-process-259-c3a-cp3-language-quality`. Scope is local/mock API evidence only, with media artifacts, access/quota/retention, security/observability, performance, and real-browser E2E still planned/non-passing and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, public distribution, or production-readiness claim. |
| 2026-07-22 | PR `#260` merged issue `#259` C3A-CP3 language-quality at `16d8cfea1fd14050833a16cc4da827be16289911`; issue `#259` is closed, issue `#249` remains open, and the next approved action is issue `#261` C3A-CP4 executable media-artifacts on branch `phase-1-closure-process-261-c3a-cp4-media-artifacts`. Scope is local/mock API evidence only, with access/quota/retention, security/observability, performance, and real-browser E2E still planned/non-passing and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, real media binaries, public distribution, or production-readiness claim. |
| 2026-07-22 | Issue `#261` started as C3A-CP4 media-artifacts on branch `phase-1-closure-process-261-c3a-cp4-media-artifacts`; the first branch commit contains only `docs/governance/preflights/issue-261.json`. Scope is executable local/mock media-artifact evidence through the API path only, with API E2E, output-correctness, and language-quality preserved, later Checkpoint 3A probes still planned/non-passing, issue `#249` remaining open, and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, real media binaries, public distribution, or production-readiness claim. |
| 2026-07-22 | Issue `#259` started as C3A-CP3 language-quality on branch `phase-1-closure-process-259-c3a-cp3-language-quality`; the first branch commit contains only `docs/governance/preflights/issue-259.json`. Scope is executable language-quality through the local/mock API path only, with API E2E and output-correctness preserved, later Checkpoint 3A probes still planned/non-passing, issue `#249` remaining open, and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, public distribution, or production-readiness claim. |
| 2026-07-22 | Issue `#257` started as C3A-CP2 output-correctness on branch `phase-1-closure-process-257-c3a-cp2-output-correctness`; the first branch commit contains only `docs/governance/preflights/issue-257.json`. Scope is executable output-correctness through the local/mock API path only, with API E2E preserved, later Checkpoint 3A probes still planned/non-passing, issue `#249` remaining open, and no provider setup, real provider calls, paid spend, hosted deployment, public URL, cloned identity, public distribution, or production-readiness claim. |
...
## Issue 306 Heartbeat 1 B status

- A1 PR `#303` and A2 PR `#305` are merged; B is the final approved Heartbeat 1 envelope under sole authority `#302`.
- Scope: real browser through the Next rewrite, exactly two controlled submissions, public approval/ingestion, metadata-only internal exclusion, genuine snapshot restart/reopen, owner isolation, and privacy-safe exact-head evidence.
- Frozen ceiling: 15 changed files, 1,090 hand-authored changed lines, and eight surfaces; the Phase 1 checker enforces the accepted 14-path allowlist and exact branch.
...
- Issue `#359` remains open with its exact branch/worktree immutable until convergence merge and merged-main
  verification; only then may closeout mark it satisfied or superseded. That point-in-time route left Issue #150
  unchanged; its later MCP-only renewal expires `2026-08-28`. The historical pre-merge Issue #358 sentence is superseded by PR #362 at
...
## Issue 376 frontend dependency-builder security accepted state

- The expired Issue #374 BusyBox acceptance was not renewed. Issue `#376` used
...
- Exact-image evidence refreshed on 2026-08-21 shows Node v26.7.0, no shared
  ...
  reproduction rotates Next secrets without retaining build inputs. PR `#437`
  merged the exact reviewed history through
  `370c52f0a1b618758e2a9a2c28188e75384a5922`; merged-main quality passed and
  Issue `#376` is closed completed. Dependent branches may consume the repair
  only by normally merging accepted `main` and repeating their own evidence.
...
## Issue #436 backend TLS prerequisite accepted state (2026-08-21)
...
the obsolete CPython Docker-shape contract to the exact route. PR `#437` merged
the exact reviewed history through
`370c52f0a1b618758e2a9a2c28188e75384a5922`; merged-main quality passed and
Issue `#436` is closed completed. Child B preserves its original 89-commit
history and consumes this prerequisite only through a normal merge; the
prerequisite does not rebase, rewrite, or activate it.

The accepted backend image uses verified CPython 3.13.15 source and exact
...
Issue #376 supplied the separate frontend minimal-runtime repair. Both
prerequisites passed unchanged full Stage 8 scanner consensus together and are
...
PR `#462` merged T04 and its inherited hosted-parity corrections at
