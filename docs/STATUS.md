...
- Current repo mode: Final Review has merged; Phase 1 Closure is active,
  ...
  reconciliation has merged through issue `#198` and PR `#199`, mutable
...
- Out-of-band GitHub reconciliation: PR `#45` merged the Final Review artifacts at
  ...
  quality run `29689512124` passing; live GitHub also shows issue
...
- Issue `#192` is closed through merged PR `#193`, issue `#194` is closed
  ...
  `#197`; issue `#198` is closed through merged PR `#199`; those status
  reconciliations are no longer the current action.
