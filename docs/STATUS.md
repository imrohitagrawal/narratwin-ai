...
| SSV1-NEXT | next-action | issue #315 | pr-product-context-gate-complete | pr-product-context-gate-complete | Issue #315 requires self-contained product and end-goal context for every non-trivial PR and makes missing, issue-only, link-only, generic, or production-overclaim context fail `policy-gates`. It explains each contribution against the end-to-end demo and eventual production path while independent reviewers retain truth and strategic judgment. Runtime and production authorization remain unchanged; Issue #280 repair, Heartbeat 3, Product Mode 2/#20, providers, hosting, deployment, private data, and protected work remain unauthorized. |
...
| `#313` | Decision complete when reviewed and merged | Issue #280 repair feasibility and independent semantic oracle | Defines the audience-collapse defect, schema-closed independent proposition oracle, exact semantic thresholds, architecture alternatives, and ADR `0044` semantic-frame GO candidate. Runtime repair remains NO-GO until a separate controlling issue commits oracle-backed behavioral RED; PR #299 and all existing forensic evidence remain unchanged. |
| `#315` | Governance gate complete when reviewed and merged | Self-contained product and end-goal PR context | Requires non-trivial PRs to explain the end goal, current state, problem, exact changes, done state, expected and prohibited outcomes, demo/production-path impact, remaining gap, and reviewer pass/fail validation in plain English. Issue references and links remain supplemental. Runtime and production authorization remain unchanged. |
...
3. Treat Issue #300 and merged PR #301 as completed historical
   negative-containment evidence. Issue #313 defines architecture feasibility
   and the independent oracle contract only. Runtime repair remains NO-GO until
   a separate controlling issue freezes executable oracle-backed RED,
   implementation scope, tests, and approval; PR #299 and existing forensic
   evidence remain unchanged and issue `#249` remains open.
4. Keep production,
