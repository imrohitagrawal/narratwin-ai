...
| `#127` | Open | Phase 1 follow-on | `CH-11` SLO and error-budget contract for `OPS-SLO-001`; scope is limited to reviewed threshold semantics, executable now versus advisory-only classification, manual review contract burn policy, and release-breach actions bound to merged `CH-10` metric names. It does not claim dashboard/query execution, alert routing, first-hour watch, successful restore drill evidence, rollback communications evidence, or issue `#39` closure. |
| `#128` | Open | Phase 1 follow-on | `CH-10` production metrics contract for `OPS-METRICS-001`; scope is limited to shared metric names plus executable emission points on the ACID/CAS kernel, local restore-adjacent state loader, and rollback block boundary. It does not claim SLO closure, dashboards/alerts, first-hour watch, successful restore drill evidence, rollback communications evidence, or issue `#39` closure. |
