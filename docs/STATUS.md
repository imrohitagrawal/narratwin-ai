...
- Current implementation permission: Phase 1 Closure governance only. Demo
  ...
  post-merge main quality workflow run `30095714825` passing. PR D is the
  current exact UI/browser demo slice for the PR C local/mock endpoint:
  `/api/v1/checkpoint3/issue280/local-e2e-demo`. PR D does not implement the
  final issue `#280` closure gate and does not prove arbitrary real-world
...
| `#289` | Open | Security unblock for PostCSS audit and Stage 8 gate drift | Combined unblock branch `phase-1-closure-process-289-security-postcss-stage8-gate-unblock` intentionally includes the PostCSS dependency metadata remediation, required ADR/traceability/third-party notice updates, and the inherited Stage 8 quality-marker drift repair so required CI can pass atomically. It keeps PR `#284` draft and untouched, keeps issues `#249` and `#280` open, and authorizes no product/runtime code, provider setup, paid spend, hosted/public demo, Next downgrade, production-readiness claim, or issue-closing wording for `#249` or `#280`. |
...
- Issue `#289` tracks the combined security unblock for the PostCSS advisory
  ...
  metadata plus checker/docs/tests/status only, keeps PR `#284` draft and
  untouched, and keeps issues `#249` and `#280` open.
