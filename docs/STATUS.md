...
| `#150` | Active | Security follow-up | OWNER selected a short MCP-only renewal: upgrade isolated Semgrep to `1.172.0`, use upstream fixed Click `8.4.2` without a Click override, retain only `mcp==1.28.1`, and expire on `2026-08-28`. Full security review, CI, PR, and merged-main verification remain required; release/production stays No-Go. |
| `#428` | Active, converged with `#150` | Security prerequisite | Current audit expansion makes transitive Nano ID `3.3.17` High severity. The OWNER-approved protected PR refreshes only official `3.3.18` while completing the bounded Semgrep renewal; frozen #427 remains inactive until joint merged-main acceptance. No product/release/production posture changes. |
