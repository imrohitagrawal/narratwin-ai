...
- Out-of-band GitHub reconciliation: PR `#45` merged the Final Review artifacts at
  ...
  `#106`; issue `#107` is open for `CH-03` Stage 4 durable graph; and issues
...
| `#107` | Open, in progress | Phase 1 follow-on | `CH-03` Stage 4 durable graph execution for `DUR-STAGE4-001` is started on branch `phase-1-closure-39-ch-03-stage4-durable-graph`; scope is limited to project/document/chunk/run/evaluation durable graph metadata, replay/conflict behavior, outbox binding, branch-scope guardrails, and supporting ADR/status/traceability updates while issue `#39` remains open. |
| `#111` | Open, in progress | Phase 1 follow-on | `CH-16` consent capture is started on branch `phase-1-closure-39-ch-16-consent-capture`; scope is limited to durable affirmative synthetic-avatar consent records, scoped/idempotent consent replay, restore hardening, Stage 7 durable consent gating, and supporting ADR/status/traceability updates while issue `#39` remains open. |
