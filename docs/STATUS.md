...
- Current implementation permission: Phase 1 Closure planning only for issue
  `#225`, limited to Demo Phase 0 source facts, hosted controlled real-media
  demo boundary definition, checkpoint sequencing, failure-matrix categories,
  and review expectations. No Phase 2 feature work, external provider
  enablement, provider SDKs, provider keys, hosted deployment, production
  release tag, real audio/video generation, cloned identity implementation,
  public synthetic-media distribution claim, or Product Mode 2 work is
  permitted by issue `#225`. Issue `#39` media/provider/security rows are
...
- Current repo mode: Final Review has merged; Phase 1 Closure is active,
  ...
  and PR `#224` have merged and closed, issue `#225` is the active Demo Phase
  0 planning issue for hosted controlled real-media source facts and plan,
...
| `#225` | Open | Demo Phase 0 planning | Active planning issue for the hosted controlled real-media demo path. It authorizes source facts, cost/terms research, checkpoint sequencing, failure-matrix categories, and fan-out review expectations only; no runtime, provider SDK, provider key, hosted deployment, real audio/video generation, cloned identity implementation, Product Mode 2, public synthetic-media distribution, or production-readiness scope is authorized. |
...
- Issue `#223` is closed through merged PR `#224`; it completed the terminal
  ...
  next-work pointer.
- Issue `#225` is open as the active Demo Phase 0 planning issue for the hosted
  controlled real-media demo path. It records the demo contract, source-fact
  strategy, provider/local-model analysis, checkpoint sequencing, failure
  matrix, and fan-out review requirements before implementation.
...
3. Complete issue `#225` through a Demo Phase 0 planning PR that documents the
   hosted controlled real-media demo contract and keeps implementation
   sequential: Checkpoint 1 first, then Checkpoint 2 after review approval.
