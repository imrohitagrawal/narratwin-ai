...
| `#308` | Open | Heartbeat 2 authority | Direct owner authorization for a two-PR post-Checkpoint-B curated-source local reviewer demo. PR A evidence integrity is active; PR B is blocked until PR A exact-head approval, merge, and closeout. This is not Product Mode 2 and does not authorize providers, real media, hosting, deployment, production, private data, or #20. |
...
3. Complete PR #301 negative forensic containment without treating Issue #280
   as fixed. The former PR E closure plan is historical and falsified. A real
   runner, semantic oracles, and product repair require separate later issues,
   branches, PRs, RED tests, and approvals; issue `#249` remains open.
4. Keep production,
...
- PR A is active on exact branch `phase-1-closure-308-heartbeat2-evidence-contract`; owner-authorized Contract reset 4 raises only its line ceiling to 8 files, 900 charged lines, four surfaces.
- PR B remains blocked pending PR A exact-head review, merge, verification, synchronization, and closeout.
- Aggregate reset envelope: 15 unique files, 1,800 charged lines, eight surfaces; PR B is capped at 12 files / 900 lines / six surfaces; synthetic fixtures and local/mock media only.
