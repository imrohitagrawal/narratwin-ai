...
- Current repo mode: Final Review has merged; Phase 1 Closure is active,
  ...
  through issue `#206` and PR `#207`, CH-M1-02 real-stack local evidence is the
  active serialized issue `#208`, issue `#209` tracks the directly related local
  Phase 1 Closure quality-dispatch clarification, mutable current-state
  authority is normalized through the StatusStateV1 table below, and release
  posture is No-Go.
...
| `#208` | Open | Product Mode 1 CH-M1-02 child | Active controlled local/mock real-stack evidence issue for browser -> frontend -> backend -> Compose evidence with no application API interception. It must not authorize Product Mode 2, real audio/video, external providers, hosted/public launch, public distribution, or production release posture changes. |
| `#209` | Open | Phase 1 Closure local quality follow-up | Directly related governance/local-quality issue included with `#208` to make plain local `make quality` on `main` unambiguous while preserving Stage 8 and CI policy behavior. |
...
- Issue `#208` is the active Product Mode 1 `CH-M1-02` step: real-stack local
  browser-to-frontend-to-backend-to-Compose evidence with no intercepted
  application APIs.
- Issue `#209` is included only for the directly related local Phase 1 Closure
  top-level quality behavior on `main`.
...
3. Complete issue `#208` CH-M1-02 in one consolidated PR that also addresses
   directly related issue `#209`, then continue the controlled local mock-demo
   path only as serialized issue-linked PRs: Checkpoint A, CH-M1-03 through
