...
- Forensic baseline: evidence at exact head
  ...
  or C3B work may be inferred from that historical closure. Issue `#317`
  authorizes and completes only semantic repair slice 1: the deterministic
  Spanish `STANDARD` cohort and independent oracle described below.
...
| `#317` | Bounded slice complete when reviewed and merged | Issue #280 semantic repair slice 1 | Adds an implementation-independent executor and frozen synthetic proposition truth, then a runtime-owned semantic frame for Spanish `STANDARD` across all seven audiences. The exact fixture is visible through API, replayed storage, artifacts, citations, and browser output; unsupported fixture language, depth, or clauses refuse. Issue #280, arbitrary multilingual quality, providers, hosting, production, and release remain unclaimed. |
...
3. Treat Issue #300 and merged PR #301 as completed historical
   ...
   Issue #317 completes the first bounded repair slice only. Expand Issue #280
   coverage through separately authorized cohorts; PR #299 and existing
