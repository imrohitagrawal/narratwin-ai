...
- Current repo mode: Final Review has merged; Phase 1 Closure is active,
  ...
  merged through issue `#194` and PR `#195`, mutable current-state authority is
...
- Out-of-band GitHub reconciliation: PR `#45` merged the Final Review artifacts at
  ...
  quality run `29684509339` passing; live GitHub also shows issue
...
- Issue `#192` is closed through merged PR `#193`, and issue `#194` is closed
  through merged PR `#195`; those status reconciliations are no longer the
  current action.
