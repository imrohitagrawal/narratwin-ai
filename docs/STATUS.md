...
- Current repo mode: Final Review has merged; Phase 1 Closure is active,
  ...
  `#221` and PR `#222`, issue `#223` / PR `#224` is the active terminal
  post-PR-222 loop-breaker for status-only reconciliation,
...
| SSV1-NEXT | next-action | #223/#224 | terminal-status-loop-breaker-active | terminal-status-loop-breaker-active | Issue #223 and PR #224 are the active terminal post-PR-222 loop-breaker; future issue-completing PRs must finalize docs/STATUS.md in the same PR with the post-merge target state and next-work pointer, while ordinary post-merge facts are recorded in PR/issue comments and not in successor status-only PRs. |
...
| `#223` | Open | Post-PR-222 terminal loop-breaker | Narrow repository-ledger and process-policy follow-up to record PR `#222` merge state, issue `#221` closure, post-merge main quality workflow evidence, and the terminal rule that future issue-completing PRs finalize `docs/STATUS.md` in the same PR while routine post-merge facts are recorded in PR/issue comments instead of successor status-only PRs. No runtime, Product Mode 2, real media, providers, hosted/public launch, public distribution, or production-readiness scope is authorized. |
...
- Issue `#223` / PR `#224` is the active terminal post-PR-222 loop-breaker.
  After PR `#224`, future issue-completing PRs must finalize `docs/STATUS.md`
  in the same PR with the intended post-merge target state and next-work
  pointer. Routine post-merge facts must be recorded in PR/issue comments and
  must not create another standalone status-only reconciliation PR.
...
3. Complete issue `#223` / PR `#224` as the terminal post-PR-222
   loop-breaker. After it merges, record PR `#224` merge facts in PR/issue
   comments, do not open a `post-PR-224 status reconciliation` issue or PR, and
   bundle any future `docs/STATUS.md` cleanup into the next material
   repository change.
