...
| `#109` | Open, in progress | Phase 1 follow-on | `CH-07` Stage 6 durable replay execution for `DUR-STAGE6-001` is started on branch `phase-1-closure-39-ch-07-stage6-durable-replay`; scope is limited to Stage 6 durable replay state, source/evaluation provenance binding, metadata/checksum validation, terminal idempotency replay, checksum-based dedupe, branch-scope guardrails, and supporting API/status/traceability updates while issue `#39` remains open. |
