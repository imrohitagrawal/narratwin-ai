...
- Issue `#213` has in-branch CH-M1-03, CH-M1-04, CH-M1-05, and CH-M1-06
  implementation/docs updates for the local/mock path, plus Checkpoint A
  baseline evidence and Checkpoint B local real-stack evidence. Latest-head
  human approval, post-merge verification, and controller issue `#155` closure
  preparation are still pending; do not close or prepare closure wording for
  `#155` yet.
