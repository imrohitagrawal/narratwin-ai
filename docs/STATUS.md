...
| `#287` | Open | Governance quality-gate drift repair | Narrow inherited Stage 8 quality-marker drift repair for dependency-security remediation. Scope is checker/docs/tests/status only on branch `phase-1-closure-process-287-stage8-quality-gate-drift`; it keeps PR `#286` dependency-only, keeps PR `#284` draft and untouched, keeps issues `#249` and `#280` open, and authorizes no product/runtime code, provider setup, paid spend, hosted/public demo, frontend dependency metadata change, Next downgrade, production-readiness claim, or issue-closing wording for `#249` or `#280`. |
...
- Issue `#287` tracks the narrow governance repair for inherited Stage 8
  quality-marker drift that blocks dependency-security PR `#286`; scope is
  checker/docs/tests/status only, keeps PR `#286` dependency-only, keeps PR
  `#284` draft and untouched, and keeps issues `#249` and `#280` open.
...
| Date | Change |
|---|---|
| 2026-07-24 | Issue `#287` started on branch `phase-1-closure-process-287-stage8-quality-gate-drift` to repair inherited Stage 8 governance quality-marker drift blocking dependency-security PR `#286`. Scope is limited to checker/docs/tests/status alignment: mandatory container scanning remains bound to Trivy, Grype, or the pinned Dockerized scanner path while Docker Scout remains optional; the Stage 6 voice-provider JSON-manifest marker is documented; PR `#286` remains dependency-only; PR `#284` remains draft and untouched; issues `#249` and `#280` remain open; and no product/runtime code, frontend dependency metadata, Next downgrade, provider setup, paid spend, hosted/public demo, public distribution, production-readiness claim, or issue-closing wording for `#249` or `#280` is authorized. |
