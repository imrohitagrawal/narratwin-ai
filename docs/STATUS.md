...
| `#141` | Open, architecture recorded; human approvals blocked | Phase 1 follow-on | Selects Amazon RDS for PostgreSQL 17.10 Multi-AZ plus versioned S3 artifact/control buckets in `ap-south-1`; defines Stage 4/6/7 state/object ownership, contiguous integrity-linked deletion/revocation journal, restricted catalog/reviewer export, restore landing-zone/target lifecycle, scoped IAM/KMS/OIDC/network access, retention/cleanup escalation, immutable-cutoff RPO calculation, and acceptance handoffs for `#142`-`#149`. No environment, backup/version source, restore target, or restore evidence has been verified. Cost/account/region, role assignment, and same-account/shared-RDS-key Security exception approvals remain blocked. |
...
| `#143` | Open, depends on `#141`, `#142` | Phase 1 follow-on | Move Stage 4/6/7 business state onto PostgreSQL; publish approved bytes through an S3-version-first compensation contract; deliver contiguous integrity-linked deletion events with gap/retry/orphan/unavailable-object evidence. |
| `#144` | Open, depends on `#141` and Security approval | Phase 1 follow-on | Provision the private Multi-AZ RDS source and isolated restore landing zone/template plus distinct source/restore/control S3 buckets through reviewed IaC. PITR later creates a new target DB; no target or resource is repo-verified now. |
| `#145` | Open, depends on `#141`, `#144` | Phase 1 follow-on | Configure 14-day RDS PITR, at least 15-day S3 version retention, unique-key/version-aware Object-Locked journal, integrity manifest, KMS safeguards, restricted catalog and reviewer-export boundary; no backup/version source is currently evidenced. |
...
| `#147` | Open, depends on `#144`, `#145`, `#146` | Phase 1 follow-on | Implement the not-yet-executed PITR/exact-S3-Version runbook, explicit target parameters, journal/revocation handoff, source deny, target-only cleanup authority, deadline automation/escalation and next-exercise block. |
| `#148` | Open, depends on `#141`, `#144`, `#145`, `#146`, `#147` | Phase 1 follow-on | Implement RDS/S3/journal/cleanup observability, restricted-to-reviewer evidence export, CH-12 alert handoff, and immutable-cutoff RTO/RPO calculations that invalidate negative or mismatched watermarks. |
| `#149` | Open, depends on `#141`-`#148` | Phase 1 follow-on | Independently review actual environment/tooling/calculation-test readiness and approvals without recording actual restore or RTO/RPO results; must leave issue `#126`, `DUR-RESTORE-001`, and issue `#39` open for the later exercise. |
