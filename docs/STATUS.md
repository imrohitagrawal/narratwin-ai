...
| `#141` | Open, architecture recorded; human approvals blocked | Phase 1 follow-on | Selects Amazon RDS for PostgreSQL 17.10 Multi-AZ plus versioned S3 artifact/control buckets in `ap-south-1`; defines Stage 4/6/7 state/object ownership, append-only deletion journal, backup/catalog/restore-target ownership, IAM/KMS/secrets/retention, invalidating RPO calculation, and acceptance handoffs for `#142`-`#149`. No environment, backup/version source, restore target, or restore evidence has been verified. Cost/account/region, role assignment, and same-account/shared-RDS-key Security exception approvals remain blocked. |
...
| `#143` | Open, depends on `#141`, `#142` | Phase 1 follow-on | Move Stage 4/6/7 business state onto PostgreSQL, approved bytes through a versioned-object adapter, and committed deletion events into the independent journal with replay/integrity evidence. |
| `#144` | Open, depends on `#141` and Security approval | Phase 1 follow-on | Provision private RDS source/target plus distinct source/restore/control S3 buckets and isolation through reviewed IaC; no such resources are repo-verified now. |
| `#145` | Open, depends on `#141`, `#144` | Phase 1 follow-on | Configure 14-day RDS PITR and S3 version retention, control-bucket Object Lock, and the sanitized backup/object/journal catalog; no backup/version source is currently evidenced. |
| `#146` | Open, depends on `#141`, `#143` | Phase 1 follow-on | Create the synthetic Stage 4/6/7 database/object seed, deletion-journal event, server commit sequence, checksums, and validator for later use against `#144`. |
| `#147` | Open, depends on `#144`, `#145`, `#146` | Phase 1 follow-on | Implement the RDS plus exact-S3-Version restore runbook, journal-derived re-delete handoff, and source/target/cleanup safety controls without executing the final drill. |
| `#148` | Open, depends on `#141`, `#144` | Phase 1 follow-on | Implement RDS/S3/journal restore observability, sanitized evidence export, and RTO/RPO calculations that invalidate negative or mismatched watermarks. |
