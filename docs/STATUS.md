...
- Out-of-band GitHub reconciliation: PR `#45` merged the Final Review artifacts at
  ...
  `af7215a5ceb7cefa81306773c1cfa8260435291e`; and issues `#43`, `#44`,
...
| `#123` | Open | Phase 1 follow-on | `CH-09` technical rollback compatibility for `DUR-ROLLBACK-001`; scope is limited to runner-persisted rollback compatibility state, previous-code-on-expanded-schema compatibility proof resolved through a reviewed out-of-band proof source, explicit unsafe-rollback blocking, forward repair, schema-version/compatibility-window enforcement, no false down-migration support claim, and supporting migration/traceability/status updates while issue `#39` remains open. |
| `#125` | Open | Phase 1 follow-on | Local restore-integrity drill for `DUR-RESTORE-001`; scope is limited to the existing optional file-backed Stage 4/6/7 state, copied-file checksum parity, restored record-count parity, and replay-safe idempotent re-execution after restore. It does not claim production backup/restore, restore metrics, RTO/RPO proof, retention/re-delete behavior, or issue `#39` closure. |
| `#126` | Open | Phase 1 follow-on | `CH-14` production-like restore drill for `DUR-RESTORE-001`; scope remains above the local `#125` drill and still requires production-like restore evidence, timings, and operational review while issue `#39` remains open. |
| `#127` | Closed | Phase 1 follow-on | `CH-11` SLO and error-budget contract for `OPS-SLO-001` completed through merged PR `#134`; scope remained limited to reviewed threshold semantics, executable now versus advisory-only classification, manual review contract burn policy, and release-breach actions bound to merged `CH-10` metric names without claiming downstream operational closure. |
| `#128` | Closed | Phase 1 follow-on | `CH-10` production metrics contract for `OPS-METRICS-001` completed through its merged child PR; scope remained limited to shared metric names plus executable emission points on the ACID/CAS kernel, local restore-adjacent state loader, and rollback block boundary without claiming downstream operational closure. |
| `#129` | Open | Phase 1 follow-on | `CH-13` first-hour watch for `OPS-WATCH-001`; downstream of the merged CH-10/CH-11 contract work. |
| `#130` | Open | Phase 1 follow-on | `CH-12` dashboards and alerts for `OPS-ALERT-001`; downstream of the merged CH-10/CH-11 contract work. |
| `#131` | Open | Phase 1 follow-on | `CH-21` retention and erasure for `SEC-RETENTION-001`; still open. |
| `#132` | Open | Phase 1 follow-on | `CH-22` untrusted replayed input for `SEC-UNTRUSTED-001`; still open. |
