...
```text
Read-only review. Do not edit files. Review Stage 2 for portability, performance, observability, cost controls, CI quality gates, and implementation-readiness. Focus on docs/PORTABILITY_STRATEGY.md, docs/OBSERVABILITY_AND_COST.md, docs/ARCHITECTURE.md, docs/API_CONTRACT.md, docs/DATA_MODEL.md, docs/STAGE2_ARCHITECTURE_CONTRACT.json, docs/STAGE2_HUMAN_REVIEW_CHECKLIST.md, docs/STAGE2_REVIEW_PROMPT_PACK.md, docs/QUALITY_GATES.md, .github/workflows/quality-gates.yml, scripts/quality/check_stage2_docs.py, scripts/guardrails_check.py, .env.example. Return actionable findings only, ordered by severity, with file:line references where possible. Classify REQUIRED before merge vs RECOMMENDED later. Look for missing budgets, queue/backpressure semantics, cache invalidation, deterministic local mode, CI enforceability, review-process gaps, and portability gaps that could block Stage 4.
```
