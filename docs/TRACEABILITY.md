...
| `backend/app/stage6.py` and `tests/unit/test_stage6_multilingual.py` | RR-029 direct domain-service rejection of blank glossary terms, secret-like glossary rejection before provider calls, and Stage 6 idempotent terminal semantic failures | Stage 8 / `#13` | Hardened |
...
| `docs/RELEASE_CHECKLIST.md`, `docs/RUNBOOK.md`, `docs/RELEASE_READINESS_REVIEW.md`, and `docs/REVIEW_RIGOR_RETROSPECTIVE.md` | release checklist, rollback notes, runbook, known limitations, multi-worker block, real video export/license posture block, persistent synthetic-media consent/provenance block, source-run based avatar rendering decision, and durable review-rigor remediation guidance | Stage 8 / `#13` | Added |
