...
| `backend/app/storage/postgres_state.py`, `backend/app/storage/__init__.py`, `tests/unit/test_postgres_state.py`, and `docs/ADR/0017-ch06-committed-outbox.md` | `CH-06` committed outbox kernel for `DUR-OUTBOX-001`, including same-transaction state/event writes, dispatcher acquire/retry/success/fail transitions, at-least-once redelivery, and consumer dedupe primitives for issue `#96` | Phase 1 Closure / `#96` | Added |
...
| 2026-07-11 | `#96` | Start CH-06 committed outbox execution | Extends the storage kernel with same-transaction outbox writes, dispatcher acquire/retry/success/fail transitions, at-least-once redelivery, focused `CTX2-OUTBOX-EVID-001` RED/GREEN tests, and consumer dedupe primitives without widening into `CH-04`, `CH-05`, or runtime dispatch integrations. |
