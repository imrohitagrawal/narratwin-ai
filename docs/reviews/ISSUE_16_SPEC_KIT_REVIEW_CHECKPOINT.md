...
| Exact route/budget was prose-only | Temp forbidden path and generic GPF adapter both returned no finding; co-mutating the artifact and diff also passed; detached hosted PR checkout rejected the correct event branch. | REQUIRED_CONTRACT | checker writer | Second blocker class stopped work; RED then fixed-base, trusted local/hosted branch, immutable path/forbidden, and charged-budget oracle corrected the shared root cause. |
...
`REQUIRED_CONTRACT` remains in the candidate. The live oracle passes locally;
independent exact-head verification remains required before approval.
