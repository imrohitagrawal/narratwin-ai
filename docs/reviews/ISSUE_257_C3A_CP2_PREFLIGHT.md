...
| `C3A-CP2-FM-002` | Correct-looking text with missing or mismatched evidence binding | Local API run fails when a controlled provider emits the required Atlas sentence with a nonexistent runtime chunk binding; no accepted script is exposed | test | `tests/acceptance/test_checkpoint3_output_correctness.py::test_checkpoint3_output_correctness_rejects_correct_text_without_evidence_binding` |
