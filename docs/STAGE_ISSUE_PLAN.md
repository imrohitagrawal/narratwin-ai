...
cap. Independent visual/provenance/privacy review accepted the one-attempt Raj
and Myra candidates. Their bounded metadata-free WebPs are registered through a
...
`phase-1-closure-280-c3a-r3-pr-e-arbitrary-demo-closure` is historical and has
no live implementation or closure authority. Its evidence head
`f93653e8a11e697c88766b207fb01c18662339d6` is retained only as falsifying
evidence. The former PR E allowlist, including `scripts/guardrails_check.py`,
backend/frontend paths, acceptance/contract tests, and Playwright paths, does
...
Allowed scope is the exact 11-path governance, forensic-verifier, matrix, and
focused-test allowlist in `scripts/quality/check_phase1_closure_docs.py`.

Backend, frontend, provider, RAG, avatar, database, Docker, workflow, and
product/runtime implementation are forbidden. Evidence head
`f93653e8a11e697c88766b207fb01c18662339d6` establishes that Issue `#280` is
not fixed. Its GitHub closure, Issue `#298`, and PR `#299` remain unchanged
forensic history. The command intentionally exits nonzero and is not a required
green CI job.
...
PR A uses exact branch `phase-1-closure-308-heartbeat2-evidence-contract`, at most nine files, 1,600 charged lines, and five surfaces under owner-authorized Contract reset 6. Its exact allowlist adds `.github/workflows/ci.yml` to ADR `0043`, `docs/PHASE_PLAN.md`, `docs/QUALITY_GATES.md`, this plan, `docs/STATUS.md`, the H2 evidence verifier, the Phase 1 checker, and its existing unit-test module. Local verifier success proves semantics/privacy only; execution acceptance requires the exact-head required CI job, zero-exit runner record, `CI_EXECUTION_BOUND`, success-only artifact, and external run/artifact metadata reconciliation.

PR B may start only after PR A exact-head approval, merge, and closeout. It uses exact branch `phase-1-closure-308-heartbeat2-curated-reviewer-demo`, at most 12 files, 900 charged lines, and six surfaces. Across both PRs the reset ceiling is 15 unique files, 2,500 charged additions plus deletions, and eight surfaces. PR A uses synthetic packets only to prove verifier/workflow behavior; genuine owned-process browser evidence belongs to PR B.

The reset-6 canonical comment database ID is `5121265229`. Issue #8, #308, and that comment must re-resolve without digest or identity drift before review and merge. Backend/API/storage/provider/deployment/hosting/production/private-data/Q&A work is forbidden; any essential unlisted path, third PR, or budget expansion stops H2.
...
## Issue #401 exact Python dependency-security route
...
PDF parsing is not reachable from product code and PDF uploads stay rejected;
Markdown/text remains the supported Cut 1 ingestion path. This reachability
fact does not make the installed 6.14.2 audit-clean. PR #400 may continue
isolated work but must not merge until #401 passes exact-head review, approval,
...
provider, spend, media, deployment, publication, release, and production
restrictions. The route is capped at 600 additions plus deletions, with exact
per-path limits recorded in `docs/QUALITY_GATES.md`. Missing or outside paths,
lookalike branches, base drift, and budget excess fail closed. Both plain and
policy-only quality commands must pass before review.

## Issue #435 — completed adversarial-convergence framework v1

Issue `#435` is closed as completed through merged PR `#453` at
`59db96aaab6c4e75b12d134dc9b02330c5a982ac`; post-merge workflow
`33090778982` passed. Its finite framework remains separate, governance-only,
and nonactivating with authority effect `NO_AUTHORITY_EFFECT`. Historical C1–C4
route, RED, review, mutation, and closeout evidence remains in Issue `#435`, the
