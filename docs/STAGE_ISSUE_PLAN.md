...
PR A uses exact branch `phase-1-closure-308-heartbeat2-evidence-contract`, at most eight files, 1,300 charged lines, and four surfaces under owner-authorized Contract reset 5. Its exact allowlist is ADR `0043`, `docs/PHASE_PLAN.md`, `docs/QUALITY_GATES.md`, this plan, `docs/STATUS.md`, the H2 evidence verifier, the Phase 1 checker, and its existing unit-test module.

PR B may start only after PR A exact-head approval, merge, and closeout. It uses exact branch `phase-1-closure-308-heartbeat2-curated-reviewer-demo`, at most 12 files, 900 charged lines, and six surfaces. Across both PRs the reset ceiling is 15 unique files, 2,200 charged additions plus deletions, and eight surfaces. PR A uses synthetic packets only to prove verifier behavior; genuine owned-process browser evidence belongs to PR B.
