...
| LRN-003 | Governance documents do not enforce themselves; high-risk changes must produce an adapted invariant matrix, source-fact table, false-pass tests, and reviewer signoff before implementation begins. | Phase 1 Closure / `#39` | [Engineering Process RCA](ENGINEERING_PROCESS_RCA.md), [Review Rigor Retrospective](REVIEW_RIGOR_RETROSPECTIVE.md), [New Project Engineering Playbook](templates/NEW_PROJECT_ENGINEERING_PLAYBOOK.md) | PR owner and reviewer | PR preflight evidence, quality gates, sub-agent review protocol, new-project playbook | Every high-risk requirement, guardrail, release, durability, privacy, security, or governance PR | Promoted to new-project playbook and governance learning | Active | Treat governance docs as executable entry criteria: no code until the adapted matrix and negative tests exist, and stop the PR if review repeatedly discovers new defect classes. |
