...
| Creatify Aurora | `PROVED` | `CONDITIONAL`: current live direct API says 5 min, while older indexed official pages said 60 s; fal limit is undocumented | `UNKNOWN`: request exposes no explicit aspect control | `PROVED` | Broad content license; fal adds a processor and public-file constraints | `FULL_LENGTH_QUALITY_CHALLENGER` |
...
| Runway Avatar Video API | `PROVED` for one-image custom avatar plus uploaded audio; exact immutable-audio behavior is unproved | `UNKNOWN` | `UNKNOWN` | `PROVED`: `/v1/avatars` and `/v1/avatar_videos` are documented | Self-service training/content terms need review; enterprise route possible | `API_CHALLENGER_CONDITIONAL` |
...
| `FULL_LENGTH_QUALITY_CHALLENGER` | Creatify Aurora through direct API or fal | Single synthetic image plus WAV, expressive whole-person intent, model controls, and inspectable async APIs | Current direct API says five minutes but older indexed official pages said 60 seconds; no explicit aspect control; broad content terms and fal private-input constraints remain |
| `API_CHALLENGER_CONDITIONAL` | Runway `gwm1_avatars` / Avatar Videos | A newly documented API creates a custom avatar from one image and generates avatar video from uploaded audio or text | Exact 127.7-second capacity, aspect control, resolution, immutable 24 kHz behavior, pricing for batch output, and applicable content terms remain unproved |
...
1. State which governed accounts are actually available: HeyGen, JoggAI,
   Colossyan, and fal.ai. Include Hedra only if a superseding agreement clears
   its published contract stop. An account name, tenant, project ID,
