# NarraTwin Master Program V2 — Audited Superset and Prototype-First Delivery

## Document control

- Controller: Issue #521.
- Candidate branch: `phase-1-closure-process-521-master-program-v2`.
- Frozen repository baseline: `origin/main` at
  `b6b0c05c7227428ff0841361f3970b0b2c40aa86`.
- Research and plan cutoff: 2026-09-06, Asia/Kolkata.
- Candidate state: `PROPOSED`; implementation authority: `NONE`.
...
presenters or the owner's governed Digital Twin. It preserves historical
evidence, separates engineering stages from product cuts, and makes provider
proof a prerequisite to provider-specific product code.

No provider is selected or activated. This candidate authorizes no provider,
model, host, membership, credential, subscription, upload, egress, paid call,
enrollment, biometric operation, deletion, publication, deployment, release,
...
or a repository merge cannot independently make V2 current.

The frozen audit baseline is:

- Master Program V1 Git blob:
  `2216951d9716b7c946098ab454265eafa25975bd`.
- Master Program V1 rendered SHA-256:
  `c3e3c85bb980aab4f818e80be3db5484e564423d77bc3ab6e81ba736c3af3420`.
- Five-cut roadmap SHA-256:
...
2. Every V1 state machine, threshold, prohibition, role, evidence duty,
   ...
   weakening.
3. Structural validation and a fresh independent semantic/security review.
4. Exact-byte owner approval of V2, its mapping, and its taxonomy.
5. Eligible non-author approval of the exact candidate head.
...
owner's governed Digital Twin, with citations and evaluation evidence
available behind the media.

The complete product flow is:

```text
Project creation
→ governed document upload and explicit approval
→ ingest, parse, chunk, and store
→ retrieve approved context
→ generate grounded semantic plan and script
→ bind every material claim to evidence
→ reject, clarify, or abstain on unsupported content
→ localize for selected language
→ synthesize and human-accept narration
→ bind script segments to real product screens/actions
→ generate presenter video
→ optionally generate non-authoritative scene/B-roll
→ deterministically compose video, exact audio, captions, citations, and disclosure
→ quarantine, validate, and commit artifacts
→ private browser playback, evidence review, download, and replay
→ retention, revocation, and deletion
→ optionally enter grounded interactive Q&A
```

Audience, depth, style, language, presenter, and delivery mode must materially
affect output. Static or canned variants cannot satisfy that behavior.
Generated UI must never be used as evidence of actual NarraTwin behavior.
Product walkthroughs use screen capture or screenshots from an identified real
build and state.
...
| Layer | Meaning |
|---|---|
| Stages 0–8 plus Final Review | Engineering and release permissions |
| Product Mode 1 | Prepared, pre-rendered walkthrough |
| Product Mode 2 | Grounded interactive Q&A |
| Cuts 1–6 | User-visible acceptance increments; they do not replace stages |
| Lane A T01–T08 | Controlled Cut 1 work; T01–T05 complete, T06–T08 incomplete |
| Historical Checkpoints 1/2/3A/3B/3C | Real-media and clone-evidence progression |
...
- Prepared walkthroughs are capped at 300 seconds unless a later owner-approved
  contract changes the ceiling.
- Current canonical audio is approximately 118–128 seconds and proves the
  current full-duration boundary, not a five-minute requirement.
- Interactive Q&A has a separate configurable session cap and billing model.
- NarraTwin-owned modular generation is the default; approved script and audio
  remain canonical.
- Integrated text-to-voice-to-video remains an R&D challenger until transcript,
  voice, exportable audio, timing, cost, reproducibility, retention, and deletion
  all pass.
...
- OAuth is authentication, not a generation interface.
...
`FAILED_TECHNICAL`, `REJECTED_QUALITY`, `BILLABLE_UNKNOWN`, `PROTOTYPE_ONLY`,
`SHORT_DURATION_ONLY`, `MANUAL_TOOL_ONLY`, `RESEARCH_ONLY`,
`IMPLEMENTATION_ELIGIBLE`, and `SUPERSEDED`.

| Gate | Action | Exit condition |
|---|---|---|
| PF-0 | Freeze claim, exclusions, fixture, thresholds, falsifier, risk, budget, and authority before output | Complete hypothesis; zero provider action |
| PF-1 | Verify official docs, account behavior, terms, privacy, price, formats, duration, aspects, lifecycle, retention, deletion, and balance | Every load-bearing fact proved; unknown facts block |
| PF-2 | Hash exact inputs, configuration, calibration, operations, deletion plan, and maximum exposure | Exact upload/egress/spend authority exists where required |
| PF-3 | Run smallest production-shaped proof through one selected surface | One declared create operation and one output; zero reroll/fallback |
| PF-4 | Quarantine, hash, probe, measure, human-review, and exercise deletion | Technical and human hard floors pass |
| PF-5 | Reproduce web/MCP/CLI proof through intended direct API and service auth | API output, billing, lifecycle, download, and deletion proved |
| PF-6 | Exercise next material risk: duration, aspects, language, profile, realtime, or capacity | Exact implementation boundary passes |
| PF-7 | Independent receipt review | `IMPLEMENTATION_ELIGIBLE`; only then may implementation issue start |

PF-3 may also satisfy PF-5 only when it already uses the intended production
API. PF-3, PF-5, and PF-6 answer distinct predeclared questions and are never
retries.

Default authority is zero uploads, zero create operations, and USD 0. Default
retries and controllable provider retries are zero; fallback is none;
concurrency is one; automatic top-up is off. Polling an accepted asynchronous
job is not a retry, but interval, poll count, and deadline are frozen. Timeout,
disconnect, or 5xx after possible acceptance becomes `BILLABLE_UNKNOWN`; the
...
model/revision, input mode, responsibility, language, identity class,
production surface, or independently substantiated configuration. Prompt
wording, seed, reroll, output selection, cosmetic crop, or upscale is not a
material change.

At most three shortlisted candidates may receive paid tests for one unresolved
capability. After two newly discovered blocker classes, stop experimentation,
perform root-cause analysis, and rewrite the failure matrix. A shared artifact
...
local/mock paths and provider-neutral boundaries, not real media, clones,
...
`RE-VID-LATENTSYNC-29S`, `RE-VID-SYNC2PRO-29S`, and `RE-VID-SYNC3-29S`.
...
portrait 1080×1920, independently created. Delivery is constant 30fps H.264
`yuv420p`, AAC-LC, ISO-BMFF MP4, within one frame of audio after documented
priming. Truncation, frozen/black intervals, unintended silence, watermark,
...
The golden suite has 200 cases: 100 supported, 40 boundary/ambiguity, 40
prompt-injection/stale/cross-project, and 20 unsupported/abstention. LLM judging
is supplemental and needs at least 100 human labels, agreement at least 0.85,
and Cohen's κ at least 0.80. Human-realism activation remains `NONE`. When
separately activated it uses 200 eligible viewers, six independent cells, 400
ratings per cell, 2,400 total, 20 pairs per cell, 20 ratings per pair, two
ratings per viewer per cell, at least 10,000 successful bootstrap draws, and
100,000 power simulations per cell. Equivalence is strictly greater than 0.4
and less than 0.6 under the frozen crossed viewer/pair logistic model.
Selection-study ratings cannot be reused for confirmation. Two qualified
...
integration, no-training, and retention requirements.

Plan B is presenter-dominant editorial hybrid: short accepted presenter shots,
real product recording, deterministic source-driven graphics, restrained
nonfactual B-roll, and exact continuous audio/captions. It is first prototyped
locally with rejected footage and cannot satisfy current T06 until amendment
...
|---|---|---|
| DT0 | Durable plan and threat model | V2, taxonomy migration, and Digital Twin contracts approved |
| DT1 | Provider/terms/privacy/restricted-store decision | Entity/product/API/DPA/no-training/retention/deletion/region/storage approved |
| DT2 | Consent, identity, liveness, capture plan | Owner completes official full capture without bypass |
| DT3 | Separate face and voice enrollment plus deletion rehearsal | Each independently accepted/revocable; rejected profile suspended/deleted before replacement |
| DT4 | Private 29.494-second English output | Face, voice, sync, disclosure, script, API, cost, deletion pass |
| DT5 | Full prepared and screen-guided multilingual walkthrough | Full duration, real-screen choreography, Hindi, owner acceptance pass |
| DT6 | Grounded realtime Digital Twin | Fictional Cut 3 plus clone-specific realtime gates pass |
| DT7 | Invite-only use and Cut 6 handoff | Access, takedown, monitoring, retention, disclosure, commercial prerequisites pass |

Face capture, voice capture, provider enrollment, prepared generation,
interactive generation, export, sharing, and publication each require a
separate policy. Authorization is scoped by project, tenant, operator,
audience, language, destination, purpose, expiry, provider, and model. Prepared
scripts need exact-hash approval; dynamic Q&A requires a distinct utterance
policy.
...
autonomous action, fraud/impersonation/harassment/deceptive presence, silent
...
choreography, disclosure, provenance, cost, acceptance, and deletion authority.
Provider IDs are replaceable bindings; provider audio is never automatically
canonical. Existing fictional identities and Google audio are reused rather
than regenerated because a platform bundles alternatives.
...
- Runway GWM-1: exact-audio generation demonstrated but current prepared-video
  route rejected for sync/crop/body/scene/native-resolution quality; retain only
  materially different Q&A/research hypotheses.
- Runway Act-Two: performance transfer requiring an authorized driving video
  whose cost and rights are included.
- Exact-audio presenter candidates: VEED Fabric/fal, JoggAI, Colossyan, Hedra,
  LongCat, and materially qualified Runway; shortlist at most three.
- HeyGen Avatar IV, VEED Fabric, and Creatify Aurora: preserve rejection; do not
  repeat unchanged single-photo/static-composition hypotheses.
- Lip repair: fal LatentSync, Sync 2 Pro, Sync 3, and Kling; compare total
  base-video-plus-repair accepted-output cost.
- Scene/body/B-roll: Seedance, Kling, WAN, Runway Gen-4/4.5, Veo/Flow, and
  Higgsfield-hosted models; remain non-authoritative visuals.
...
runs the same fixture; applies objective and human gates; runs the hard case;
compares total accepted-output cost; chooses primary/fallback only after direct
API evidence; freezes exact provider/model/host/account/region/profile; writes
RED contracts from observed behavior; and minimally extends current neutral
boundaries.
...
for realtime ASR/retrieval/LLM/TTS/avatar/idle/minimum/concurrency/session cost.

Normalize 29.494-second, current full duration, both aspects, six Cut 1 cells,
one 300-second walkthrough, one Digital Twin enrollment plus output, realtime at
...
`ProjectExecutionRequirementsV1`, `SecretRefV1`,
`ProviderGovernanceProfileV1`, `RunManifestV1`, `PaidOperationV1`, existing
`AvatarVideoProvider`/`TTSProvider`, `ArtifactStore` with
`QUARANTINE → VALIDATE → COMMIT`, and `DisclosureProfileV1`.

Add or formalize only in their gated increments: `SupersetMappingV2`,
`CutTaxonomyV2`, `ArchitectureHypothesisV1`,
`PrototypeQualificationReceiptV1`, `CostReconciliationV1`,
`ProviderCapabilityManifestV1`, `LanguageCapabilityManifestV1`, `MediaProbeV1`,
`WalkthroughChoreographyManifestV1`, `PresenterReferencePackV1`,
`RestrictedEvidenceStoreConfigV1`, `RestrictedEvidenceRefV1`,
`DigitalTwinConsentV1`, `DigitalTwinUseAuthorizationV1`,
...
key custody, ACL/step-up auth, audit/region, backup/Time Machine/Spotlight/cloud
sync/indexing, temp/crash files, retention/legal hold, provider-copy tracking,
purge, and anti-resurrection. FileVault local storage may be a reviewed
...
Provider-required marks are never removed, cropped, or concealed.
...
without an unexpired matching `IMPLEMENTATION_ELIGIBLE` receipt. Private media
and account data remain outside Git; hosted parity precedes activation.

G3 selects and validates the restricted evidence store; inventories the full
evidence root without exposing secrets; reconciles credential files and
rotation; builds sanitized artifact/cost ledgers; reconciles provider jobs to
debits/invoices; and separates user observations from objective acceptance.
...
main; cite an unexpired receipt; write RED tests; minimally extend neutral
interfaces; keep providers disabled in local/dev/test/CI; inject typed validated
configuration for timeouts/retries/polling/concurrency/budgets/limits/model/
...
issue/branch/base/spec/hash, premature V1 supersession, legacy Cut 5 confusion,
and prose/agent-context substitution for canonical authority.

Prototype checks reject missing/expired/wrong-host/model/account/language/aspect/
duration/web-only receipts; short proof used as full proof; lower-native media as
native 1080; retry after failed or billable-unknown jobs; cost above authority;
unapproved identity/voice/biometric egress; and missing deletion evidence.
...
as truth; unsupported/stale/cross-project content; two new blocker classes
without failure-matrix revision; or any reproduced `CRITICAL_BLOCKER` or
`REQUIRED_CONTRACT`.

Assumptions are: prior provider authorizations are exhausted; provider calls,
funding, enrollment, biometric upload, publication, and deletion remain
unauthorized; accepted presenter/audio bytes stay immutable; prepared output is
at most 300 seconds and current proofs use accepted 118–128-second audio;
realtime duration is separately configured/approved; provider facts and prices
are revalidated immediately before activation; legal/commercial conclusions
belong to responsible humans; and restricted evidence remains in approved
storage while Git contains only sanitized references.

The program is complete only when the mapping proves zero loss; V2 receives
independent semantic review and exact-byte owner approval; V2 merges through
governed hosted parity and becomes current via the separate transition; the
prototype gate is executable; evidence/security/cost baselines are reconciled;
each implementation cites a passing unexpired receipt; and no Cut/provider/
Digital Twin/commercial/public/production claim exceeds exact accepted evidence.
