...
| OUTBOX-BIND-001 | Event binding | Outbox event resource/version/payload hash matches the durable row version it describes. | A valid event row can point at a stale or unrelated resource version while tests only assert event existence. | `test_context2_outbox_binds_event_to_staged_resource_payload` | `test_context2_outbox_rejects_unmatched_resource_version`; RED confirmed before implementation because no event binding API existed. | `uv run pytest tests/unit/test_postgres_state.py -k outbox` | Runtime/state | pass |
