...
| `DELIVERING` | in-flight dispatch attempt | may transition to `SUCCEEDED`, `FAILED`, or back to `PENDING` only when the caller presents the current dispatcher identity and dispatch-attempt `lock_token`; expired `locked_until` permits reclaim but not stale completion |
...
- Dispatcher completion, retry, failure, and consumer-delivery recording must
  prove current attempt ownership with the row's `lock_token`, not only
  `locked_by`; this rejects stale same-dispatcher attempts after expiry and
  reclaim.
