...
| P1-R7 | Semgrep compatibility overrides can expire or restore advisory-bearing dependencies | P1 | `#138`, `#150`, `#158`, `#460` | Issue `#460` pins isolated Semgrep `1.175.0`, resolves upstream Click `8.4.2` and MCP `1.29.0`, and removes all tool overrides after the Issue `#150` exception expires. Strict audits, reviewed-input hashes, scan/canaries, and nonproduction posture remain mandatory. |
