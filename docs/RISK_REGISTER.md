...
| P1-R7 | Semgrep `1.168.0` has not declared compatibility with fixed Click releases | P1 | `#138`, `#150` | Semgrep is isolated from the application/runtime lock and runs with an exact tool-only Click `8.3.3` override. Root and tool audits, installed-inventory checks, exact scan coverage, canaries, and backend-image inventory fail closed. Security/repo owner acceptance is required before merge; issue `#150` tracks removal and the exception expires `2026-08-13`. |
| P1-R8 | Current stable CPython backend image has three PSF-confirmed HIGH vulnerabilities with scanner disagreement | P1 | `#151` | Grype reports `CVE-2026-11940`, `CVE-2026-11972`, and `CVE-2026-15308` as HIGH in the pinned CPython `3.13.14` runtime, while a freshly updated Trivy `0.72.0` reports zero critical/high results. The current official image resolves to the already-pinned digest, and PSF records stable releases below `3.15.0` as affected while `3.15` is beta. A passing Trivy command is not treated as proof that known risk is absent. Issue `#151` blocks #138 merge eligibility and dependent PR `#140` pending a stable patched runtime and scanner-consensus security decision. |
