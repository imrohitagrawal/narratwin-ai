...
ISSUE435_ROUTE_RED = "dcbe15d58dff5ceafe7319e2baa3302ff01b6510"
...
def inspect_issue435_repository(root: Path, branch: str) -> RouteInspection:
    ...
    if not freeze_present:
        ...
        if len(commits) != 17 or commits[-1] != head or parent is None or parent.strip() != ISSUE435_ROUTE_RED:
            findings.append(FindingCode.ROUTE_DRIFT)
    else:
        ...
        if len(commits) == 18 and commits[16] == c2_head:
            phase = "C3"
            delta = _text(_git(root, "diff-tree", "--no-commit-id", "--name-only", "-r", commits[17]))
            if delta is None or delta.splitlines() != [ISSUE435_FREEZE]:
                findings.append(FindingCode.ROUTE_DRIFT)
        elif len(commits) == 19 and commits[16] == c2_head:
            phase = "C4"
            c3_delta = _text(_git(root, "diff-tree", "--no-commit-id", "--name-only", "-r", commits[17]))
            c4_delta = _text(_git(root, "diff-tree", "--no-commit-id", "--name-only", "-r", commits[18]))
