...
def _route_head(root: Path, branch: str) -> str:
    ...
    valid = branch == event_branch == ISSUE435_BRANCH and not local.strip() and event_base != (event_head or head) and _hex_text(event_base, 40) and (not event_head or _hex_text(event_head, 40))
...
def inspect_issue435_repository(root: Path, branch: str) -> RouteInspection:
    ...
    if not freeze_present:
        ...
        parity = len(commits) == 40 and commits[-2:] == [ISSUE435_PUBLISHED_C4, head]
        ...
        if parity and commits[36] == c2_head:
            ...
            if parent is None or parent.strip() != ISSUE435_PUBLISHED_C4 or delta is None or set(delta.splitlines()) != ISSUE435_PARITY_PATHS:
                findings.append(FindingCode.ROUTE_DRIFT)
