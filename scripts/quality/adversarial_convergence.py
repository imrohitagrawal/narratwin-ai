...
def _route_head(root: Path, branch: str) -> str:
    ...
    direct = _git(root, "merge-base", "--is-ancestor", event_base, event_head) if head == event_head else None
    return event_head if valid and not local.strip() and (fields == [head, event_base, event_head] or direct is not None and direct.state == "OK" and direct.returncode == 0) else ""
def inspect_issue435_repository(root: Path, branch: str) -> RouteInspection:
    ...
    if not freeze_present:
        ...
        if len(commits) != 27 or commits[-1] != head or parent is None or parent.strip() != ISSUE435_HOSTED_C4:
            findings.append(FindingCode.ROUTE_DRIFT)
    else:
