...
ISSUE435_ROUTE_RED, ISSUE435_ROUTE_C2 = "dcbe15d58dff5ceafe7319e2baa3302ff01b6510", "8a9bdc41c63cb449afdc6bf7f806ef946a73faa2"
...
def inspect_issue435_repository(root: Path, branch: str) -> RouteInspection:
    ...
    if not freeze_present:
        ...
        if len(commits) != 18 or commits[-1] != head or parent is None or parent.strip() != ISSUE435_ROUTE_C2:
            findings.append(FindingCode.ROUTE_DRIFT)
    else:
        ...
        if len(commits) == 19 and commits[17] == c2_head:
            phase = "C3"
            delta = _text(_git(root, "diff-tree", "--no-commit-id", "--name-only", "-r", commits[18]))
            if delta is None or delta.splitlines() != [ISSUE435_FREEZE]:
                findings.append(FindingCode.ROUTE_DRIFT)
        elif len(commits) == 20 and commits[17] == c2_head:
            phase = "C4"
            c3_delta = _text(_git(root, "diff-tree", "--no-commit-id", "--name-only", "-r", commits[18]))
            c4_delta = _text(_git(root, "diff-tree", "--no-commit-id", "--name-only", "-r", commits[19]))
