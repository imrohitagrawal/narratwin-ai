...
ISSUE435_HISTORY = (ISSUE435_C1, "b099747812bcd97f812358908cb847c351190bc3", ISSUE435_REJECTED, ISSUE435_BLOCKED, "6d741aec9a2a56d54034e0092a2e24d535079517", "9bd0a2786ca41e720a275e70a2c98470a3f3aa38", "6b681b4acc419d2fa63c35862d6b6185ce82dd50", ISSUE435_H6, ISSUE435_C2_BASE, ISSUE435_C2_HARDENED, ISSUE435_C2_TYPED, ISSUE435_C2, ISSUE435_PRIOR_C2, ISSUE435_PRIOR_C3, ISSUE435_PRIOR_C4)
...
def _route_head(root: Path, branch: str) -> str:
    ...
    valid = branch == event_branch == ISSUE435_BRANCH and _hex_text(event_base, 40) and _hex_text(event_head, 40)
...
def inspect_issue435_repository(root: Path, branch: str) -> RouteInspection:
    ...
    if not freeze_present:
        ...
        if len(commits) != 16 or commits[-1] != head or parent is None or parent.strip() != ISSUE435_PRIOR_C4:
            findings.append(FindingCode.ROUTE_DRIFT)
    else:
        ...
        if len(commits) == 17 and commits[15] == c2_head:
            phase = "C3"
            delta = _text(_git(root, "diff-tree", "--no-commit-id", "--name-only", "-r", commits[16]))
            if delta is None or delta.splitlines() != [ISSUE435_FREEZE]:
                findings.append(FindingCode.ROUTE_DRIFT)
        elif len(commits) == 18 and commits[15] == c2_head:
            phase = "C4"
            c3_delta = _text(_git(root, "diff-tree", "--no-commit-id", "--name-only", "-r", commits[16]))
            c4_delta = _text(_git(root, "diff-tree", "--no-commit-id", "--name-only", "-r", commits[17]))
