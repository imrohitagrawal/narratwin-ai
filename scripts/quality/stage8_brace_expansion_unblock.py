"""Exact Stage 8 route for the Issue #360 brace-expansion security unblock."""
...
BASE, LINE_CAP = "b9a2a8cd4aa05328116565990fc30ae44592c875", 450
ALLOWED_FILES = {
...
def validate_preflight(data: dict[str, Any], failures: list[str]) -> None:
    ...
    if set(scope.get("required", ())) != ALLOWED_FILES or set(scope.get("allowed_prefixes", ())) != ALLOWED_FILES:
        failures.append("Issue #360 preflight must require exactly the authorized 14 paths.")
...
def _check_budget(root: Path, run: Callable[[list[str]], Any], head: str, failures: list[str]) -> None:
    ...
    if paths != ALLOWED_FILES or charged > LINE_CAP:
        failures.append(f"Issue #360 requires exactly 14 paths and at most {LINE_CAP} charged lines.")
