...
ISSUE516_BRANCH = "stage8-516-t06-dual-plan-strategy"
...
ISSUE507_BASE = "615298647609d2656d5e597209a8247467c71e78"
ISSUE507_TREE = "1e4834ff96bf1cdc9a6ffb353de5e97ad68dc0fc"
...
ISSUE509_BASE = "0f608af347e89c749fcb5bd6ca17a63ceccd56e7"
ISSUE509_TREE = "9418e5b13a1a1acc00371bbf51a4ec78ac50eccc"
ISSUE509_ROUTE_COMMENT = "5515713754"
ISSUE509_BODY_SHA256 = "079852878ecc9890430fe5c8de7246ec4852eb10407d4057631b38abd6b6b5a2"
ISSUE512_BASE = "60198a86b5558df5dec42c0f90cb3343f67bb286"
ISSUE512_TREE = "f58f42b1b1f069ba23d9b754988e4c023d872706"
ISSUE512_FREEZE_COMMENT = "5522069340"
...
ROUTE_ISSUES[ISSUE509_BRANCH] = 509
...
def check_exact_route(
    ...
    if branch == ISSUE516_BRANCH:
        try:
            preflight = load_json_without_duplicate_members(
                root / "docs/governance/preflights/issue-516.json"
            )
            findings = validate_governance_preflight(
                preflight,
                context={"issue_number": 516, "branch": branch, "changed_files": sorted(files)},
            )
            failures.extend(
                f"Issue #516 governance preflight failed: {item.code}" for item in findings
            )
            ...
            if not isinstance(objective, str) or any(
                ...
                for value in (ISSUE516_BASE, ISSUE516_TREE, ISSUE516_ROUTE_COMMENT)
            ):
                failures.append("Issue #516 authority drifted.")
        except (OSError, ValueError, TypeError) as error:
            failures.append(f"Issue #516 governance preflight failed closed: {error}")
    elif branch == ISSUE514_BRANCH:
        try:
            preflight = load_json_without_duplicate_members(
        ...
        try:
            ...
            findings = validate_governance_preflight(
                preflight,
                context={"issue_number": 512, "branch": branch, "changed_files": sorted(files)},
            )
            failures.extend(
            ...
            issue512_authority = (
                ...
                ISSUE512_IMMUTABLE_CORRECTION_COMMENT,
    ...
    elif branch == ISSUE509_BRANCH:
        try:
            preflight = load_json_without_duplicate_members(
                root / "docs/governance/preflights/issue-509.json"
            )
            findings = validate_governance_preflight(
                preflight,
                context={"issue_number": 509, "branch": branch, "changed_files": sorted(files)},
            )
            failures.extend(
                f"Issue #509 governance preflight failed: {item.code}" for item in findings
            )
            objective = preflight.get("objective") if isinstance(preflight, dict) else None
            issue509_authority = (
                ISSUE509_BASE,
                ISSUE509_TREE,
                ISSUE509_ROUTE_COMMENT,
                ISSUE509_BODY_SHA256,
            )
            if not isinstance(objective, str) or any(
                value not in objective for value in issue509_authority
            ):
                failures.append("Issue #509 authority drifted.")
        except (OSError, ValueError, TypeError) as error:
            failures.append(f"Issue #509 governance preflight failed closed: {error}")
    elif branch == ISSUE507_BRANCH:
        try:
            preflight = load_json_without_duplicate_members(
                root / "docs/governance/preflights/issue-507.json"
            )
            findings = validate_governance_preflight(
                preflight,
                context={"issue_number": 507, "branch": branch, "changed_files": sorted(files)},
            )
            failures.extend(
                f"Issue #507 governance preflight failed: {item.code}" for item in findings
            )
            objective = preflight.get("objective") if isinstance(preflight, dict) else None
            issue507_authority = (
                ISSUE507_BASE,
            ...
            if not isinstance(objective, str) or any(
                ...
                failures.append("Issue #507 authority drifted.")
        except (OSError, ValueError, TypeError) as error:
            failures.append(f"Issue #507 governance preflight failed closed: {error}")
    elif branch == ISSUE502_BRANCH:
        try:
            preflight = load_json_without_duplicate_members(
    ...
    try:
        ...
        if branch in {ISSUE525_BRANCH, ISSUE495_BRANCH, ISSUE479_BRANCH, ISSUE482_BRANCH, ISSUE478_BRANCH, ISSUE475_BRANCH, ISSUE459_BRANCH, ISSUE459_T03_BRANCH, ISSUE459_T05A_BRANCH,
            ...
            transitions = (
                *((run(["git", "diff", "--name-status", "-z", "--find-copies-harder",
                        ISSUE479_BASE, ISSUE479_FROZEN_HEAD, "--"]),) if branch == ISSUE479_BRANCH
                              ...
                              ISSUE494_BASE, ISSUE494_FROZEN_HEAD, "--"]),)
                   if branch == ISSUE494_BRANCH
                   else () if branch in {ISSUE525_BRANCH, ISSUE495_BRANCH, ISSUE482_BRANCH, ISSUE478_BRANCH, ISSUE475_BRANCH, ISSUE459_T03_BRANCH, ISSUE459_T05A_BRANCH,
